...
public class ComponentTFHollowTreeMedBranch extends hb
{
    int[] src;
    int[] dest;
    double length;
    ...
    protected ComponentTFHollowTreeMedBranch(final int i, final int sx, final int sy, final int sz, final double length, final double angle, final double tilt, final boolean leafy) {
        ...
        this.leafy = leafy;
        this.h = 0;
        this.g = new qc(Math.min(this.src[0], this.dest[0]), Math.min(this.src[1], this.dest[1]), Math.min(this.src[2], this.dest[2]), Math.max(this.src[0], this.dest[0]), Math.max(this.src[1], this.dest[1]), Math.max(this.src[2], this.dest[2]));
    }
    
    public void a(final hb structurecomponent, final List list, final Random rand) {
    ...
    public boolean a(final wz world, final Random random, final qc sbb) {
        final int[] rSrc = { this.src[0] - this.g.a, this.src[1] - this.g.b, this.src[2] - this.g.c };
        final int[] rDest = { this.dest[0] - this.g.a, this.dest[1] - this.g.b, this.dest[2] - this.g.c };
        this.drawBresehnam(world, sbb, rSrc[0], rSrc[1], rSrc[2], rDest[0], rDest[1], rDest[2], TFBlocks.wood.bO, 0);
        return true;
    }
    
    protected void drawBresehnam(final wz world, final qc sbb, final int x1, final int y1, final int z1, final int x2, final int y2, final int z2, final int blockValue, final int metaValue) {
