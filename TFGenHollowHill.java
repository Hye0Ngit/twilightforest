...
public class TFGenHollowHill extends TFGenerator
{
    ...
    public boolean a(final ge world, final Random rand, final int x, final int y, final int z) {
        ...
        for (int i = 0; i < this.sn; ++i) {
            final int[] dest = this.getCoordsInHill2D();
            new TFGenCaveStalactite(vz.t.bO, rand.nextDouble(), true).a(this.worldObj, rand, dest[0], this.hy + 1, dest[1]);
        }
        for (int i = 0; i < this.sn; ++i) {
            final int[] dest = this.getCoordsInHill2D();
            new TFGenCaveStalactite(vz.t.bO, rand.nextDouble() * 0.7, false).a(this.worldObj, rand, dest[0], this.hy + 1, dest[1]);
        }
        if (this.hsize == 3) {
    ...
    protected kp getTreasure(final int level) {
        if (level == 1) {
            switch (this.hillRNG.nextInt(6)) {
                case 0: {
                    return new kp(id.n, this.hillRNG.nextInt(4) + 1);
                }
                case 1: {
                    return new kp(id.av);
                }
                ...
                case 4: {
                    return new kp(id.S, this.hillRNG.nextInt(3) + 1);
                }
                default: {
                    return new kp(vz.aq, this.hillRNG.nextInt(16) + 1);
                }
            }
        }
        else if (level == 2) {
            switch (this.hillRNG.nextInt(8)) {
                ...
                case 4: {
                    return new kp(id.o, this.hillRNG.nextInt(6) + 1);
                }
                case 5: {
                    return new kp(id.az);
                }
                case 6: {
                    return new kp(id.aV, this.hillRNG.nextInt(10) + 1, this.hillRNG.nextInt(16));
                }
                default: {
                    return new kp(id.E);
                }
            ...
            if (level != 3) {
                return new kp(id.l, this.hillRNG.nextInt(16) + 1);
            }
            switch (this.hillRNG.nextInt(8)) {
                case 0:
                case 1:
