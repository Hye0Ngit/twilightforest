...
public class ComponentTFHollowHill extends StructureTFComponent
{
    ...
    public ComponentTFHollowHill(final wz world, final Random rand, final int i, final int size, final int x, final int y, final int z) {
        super(i);
        this.h = 0;
        this.hsize = size;
    ...
    public void a(final hb structurecomponent, final List list, final Random random) {
    ...
    private void generateTreasureChest(final wz world, final int x, final int y, final int z, final qc sbb) {
    ...
    protected void generateOreStalactite(final wz world, final int x, final int y, final int z, final qc sbb) {
    ...
    protected void generateStoneStalactite(final wz world, final double length, final boolean up, final int x, final int y, final int z, final qc sbb) {
        final int dx = this.a(x, z);
        final int dy = this.b(y);
        final int dz = this.b(x, z);
        if (sbb.b(dx, dy, dz) && world.a(dx, dy, dz) != ox.as.bO) {
            final Random stalRNG = new Random(world.v() + dx * dz);
            new TFGenCaveStalactite(ox.t.bO, stalRNG.nextDouble() * length, up).a(world, stalRNG, dx, dy, dz);
        }
    }
