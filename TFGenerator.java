...
public abstract class TFGenerator extends pg
{
    protected fq worldObj;
    
    public abstract boolean a(final fq p0, final Random p1, final int p2, final int p3, final int p4);
    ...
    protected byte randStone(final Random rand, final int howMuch) {
        return (rand.nextInt(howMuch) >= 1) ? ((byte)ud.y.bO) : ((byte)ud.aq.bO);
    }
    
    protected boolean isAreaClear(final fq world, final Random rand, final int x, final int y, final int z, final int dx, final int dy, final int dz) {
        boolean flag = true;
        for (int cx = 0; cx < dx; ++cx) {
            for (int cz = 0; cz < dy; ++cz) {
                final ls m = world.d(x + cx, y - 1, z + cz);
                if (m != ls.c && m != ls.b && m != ls.e) {
                    flag = false;
                }
