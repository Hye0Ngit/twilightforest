...
public abstract class TFGenerator extends ig
{
    protected ry worldObj;
    
    public abstract boolean a(final ry p0, final Random p1, final int p2, final int p3, final int p4);
    ...
    protected byte randStone(final Random rand, final int howMuch) {
        return (rand.nextInt(howMuch) >= 1) ? ((byte)yy.w.bM) : ((byte)yy.ao.bM);
    }
    
    protected boolean isAreaClear(final ry world, final Random rand, final int x, final int y, final int z, final int dx, final int dy, final int dz) {
        boolean flag = true;
        for (int cx = 0; cx < dx; ++cx) {
            for (int cz = 0; cz < dy; ++cz) {
                final p m = world.e(x + cx, y - 1, z + cz);
                if (m != p.c && m != p.b && m != p.e) {
                    flag = false;
                }
