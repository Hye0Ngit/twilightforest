...
public abstract class TFGenerator extends qt
{
    protected ge worldObj;
    
    public abstract boolean a(final ge p0, final Random p1, final int p2, final int p3, final int p4);
    ...
    protected boolean putBlockAndMetadata(final int dx, final int dy, final int dz, final int blockValue, final int metaValue, final boolean priority) {
        if (priority) {
            this.worldObj.a(dx, dy, dz, blockValue, metaValue);
        }
        else {
    ...
    protected byte randStone(final Random rand, final int howMuch) {
        return (rand.nextInt(howMuch) >= 1) ? ((byte)vz.w.bO) : ((byte)vz.ao.bO);
    }
    
    protected boolean isAreaClear(final ge world, final Random rand, final int x, final int y, final int z, final int dx, final int dy, final int dz) {
        boolean flag = true;
        for (int cx = 0; cx < dx; ++cx) {
            for (int cz = 0; cz < dy; ++cz) {
                final na m = world.d(x + cx, y - 1, z + cz);
                if (m != na.c && m != na.b && m != na.e) {
                    flag = false;
                }
                for (int cy = 0; cy < dz; ++cy) {
                    if (!world.g(x + cx, y + cy, z + cz)) {
                        flag = false;
                    }
                }
            }
        ...
        for (int cx = 0; cx < width; ++cx) {
            for (int cy = 0; cy < height; ++cy) {
                for (int cz = 0; cz < depth; ++cz) {
                    this.worldObj.a(dx + cx, dy + cy, dz + cz, blockID, meta);
                }
            }
        }
    }
