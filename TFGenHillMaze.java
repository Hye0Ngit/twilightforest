...
public class TFGenHillMaze extends TFGenerator
{
    ...
    void deadEndWebs(final int x, final int z, final int f) {
        ...
        final int dz = this.maze.getWorldZ(z);
        if (f == 0) {
            this.worldObj.g(dx + 1, dy + 0, dz + 1, ox.W.bO);
            this.worldObj.g(dx + 0, dy + 1, dz + 1, ox.W.bO);
            this.worldObj.g(dx + 2, dy + 1, dz + 1, ox.W.bO);
            this.worldObj.g(dx + 1, dy + 2, dz + 1, ox.W.bO);
            this.worldObj.g(dx + 0, dy + 0, dz + 2, ox.W.bO);
            this.worldObj.g(dx + 2, dy + 0, dz + 2, ox.W.bO);
            this.worldObj.g(dx + 1, dy + 1, dz + 2, ox.W.bO);
        }
        else if (f == 1) {
            this.worldObj.g(dx + 1, dy + 0, dz + 1, ox.W.bO);
            this.worldObj.g(dx + 1, dy + 1, dz + 0, ox.W.bO);
            this.worldObj.g(dx + 1, dy + 1, dz + 2, ox.W.bO);
            this.worldObj.g(dx + 1, dy + 2, dz + 1, ox.W.bO);
            this.worldObj.g(dx + 0, dy + 0, dz + 0, ox.W.bO);
            this.worldObj.g(dx + 0, dy + 0, dz + 2, ox.W.bO);
            this.worldObj.g(dx + 0, dy + 1, dz + 1, ox.W.bO);
        }
        else if (f == 2) {
            ...
            this.worldObj.g(dx + 1, dy + 2, dz + 1, ox.W.bO);
            this.worldObj.g(dx + 0, dy + 0, dz + 0, ox.W.bO);
            this.worldObj.g(dx + 2, dy + 0, dz + 0, ox.W.bO);
            this.worldObj.g(dx + 1, dy + 1, dz + 0, ox.W.bO);
        }
        else if (f == 3) {
            this.worldObj.g(dx + 1, dy + 0, dz + 1, ox.W.bO);
    ...
    void deadEndPainting(final int x, final int z, final int f) {
        ...
        final gl[] aenumart = gl.values();
        final String artID = aenumart[anum].A;
        mh painting = null;
        if (f == 0) {
            this.worldObj.g(dx + 0, dy + 1, dz + 2, ox.aq.bO);
            painting = new mh(this.worldObj, dx + 1, dy + 1, dz + 3, 0, artID);
            this.worldObj.g(dx + 2, dy + 1, dz + 2, ox.aq.bO);
        }
        else if (f == 1) {
            this.worldObj.g(dx + 0, dy + 1, dz + 0, ox.aq.bO);
            painting = new mh(this.worldObj, dx - 1, dy + 1, dz + 1, 3, artID);
            this.worldObj.g(dx + 0, dy + 1, dz + 2, ox.aq.bO);
        }
        else if (f == 2) {
            this.worldObj.g(dx + 0, dy + 1, dz + 0, ox.aq.bO);
            painting = new mh(this.worldObj, dx + 1, dy + 1, dz - 1, 2, artID);
            this.worldObj.g(dx + 2, dy + 1, dz + 0, ox.aq.bO);
        }
        else if (f == 3) {
            this.worldObj.g(dx + 2, dy + 1, dz + 0, ox.aq.bO);
            painting = new mh(this.worldObj, dx + 3, dy + 1, dz + 1, 1, artID);
            this.worldObj.g(dx + 2, dy + 1, dz + 2, ox.aq.bO);
        }
        if (painting != null && painting.n()) {
            if (!this.worldObj.F) {
                this.worldObj.a((nk)painting);
            }
        }
        else {
    ...
    void deadEndTrap(final int x, final int z, final int f) {
        ...
        final int dy = this.maze.worldY;
        final int dz = this.maze.getWorldZ(z);
        this.worldObj.g(dx + 1, dy + 0, dz + 1, ox.aK.bO);
        if (f == 0) {
            this.worldObj.g(dx + 1, dy - 1, dz + 2, ox.am.bO);
        }
        else if (f == 1) {
            this.worldObj.g(dx + 0, dy - 1, dz + 1, ox.am.bO);
        }
        else if (f == 2) {
            this.worldObj.g(dx + 1, dy - 1, dz + 0, ox.am.bO);
        }
        else if (f == 3) {
            this.worldObj.g(dx + 2, dy - 1, dz + 1, ox.am.bO);
        }
    }
    ...
    void deadEndTorch(final int x, final int z, final int f) {
        ...
        final int dz = this.maze.getWorldZ(z);
        if (f == 0) {
            this.worldObj.g(dx + 1, dy + 1, dz + 2, ox.aq.bO);
        }
        else if (f == 1) {
            this.worldObj.g(dx + 0, dy + 1, dz + 1, ox.aq.bO);
        }
        else if (f == 2) {
            this.worldObj.g(dx + 1, dy + 1, dz + 0, ox.aq.bO);
        }
        else if (f == 3) {
            this.worldObj.g(dx + 2, dy + 1, dz + 1, ox.aq.bO);
        }
    }
    ...
    void deadEndNook(final int x, final int z, final int f) {
        ...
        final int dz = this.maze.getWorldZ(z);
        if (f == 0) {
            this.worldObj.g(dx + 0, dy + 0, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 1, dy + 0, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 0, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 1, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 1, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 2, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 1, dy + 2, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 2, dz + 2, ox.t.bO);
        }
        else if (f == 1) {
            this.worldObj.g(dx + 0, dy + 0, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 0, dz + 1, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 0, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 1, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 1, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 2, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 2, dz + 1, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 2, dz + 2, ox.t.bO);
        }
        else if (f == 2) {
            this.worldObj.g(dx + 0, dy + 0, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 1, dy + 0, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 0, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 1, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 1, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 0, dy + 2, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 1, dy + 2, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 2, dz + 0, ox.t.bO);
        }
        else if (f == 3) {
            this.worldObj.g(dx + 2, dy + 0, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 0, dz + 1, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 0, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 1, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 1, dz + 2, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 2, dz + 0, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 2, dz + 1, ox.t.bO);
            this.worldObj.g(dx + 2, dy + 2, dz + 2, ox.t.bO);
        }
    }
    ...
    void deadEndFountain(final int x, final int z, final int f) {
        ...
        if (f == 0) {
            this.worldObj.g(dx + 1, dy + 1, dz + 2, ox.A.bO);
        }
        else if (f == 1) {
            this.worldObj.g(dx + 0, dy + 1, dz + 1, ox.A.bO);
        }
        else if (f == 2) {
            this.worldObj.g(dx + 1, dy + 1, dz + 0, ox.A.bO);
        }
        else if (f == 3) {
            this.worldObj.g(dx + 2, dy + 1, dz + 1, ox.A.bO);
        }
    }
    ...
    void deadEndLavaFountain(final int x, final int z, final int f) {
        ...
        this.worldObj.g(dx + 1, dy - 1, dz + 1, 0);
        if (f == 0) {
            this.worldObj.g(dx + 1, dy + 1, dz + 2, ox.C.bO);
        }
        else if (f == 1) {
            this.worldObj.g(dx + 0, dy + 1, dz + 1, ox.C.bO);
        }
        else if (f == 2) {
            this.worldObj.g(dx + 1, dy + 1, dz + 0, ox.C.bO);
        }
        else if (f == 3) {
            this.worldObj.g(dx + 2, dy + 1, dz + 1, ox.C.bO);
        }
    }
    ...
    void deadEndDoor(final int x, final int z, final int f) {
        ...
        this.deadEndDoorway(x, z, f);
        if (f == 0) {
            this.worldObj.d(dx + 1, dy + 0, dz + 0, ox.aE.bO, 1);
            this.worldObj.d(dx + 1, dy + 1, dz + 0, ox.aE.bO, 9);
        }
        else if (f == 1) {
            this.worldObj.d(dx + 2, dy + 0, dz + 1, ox.aE.bO, 2);
            this.worldObj.d(dx + 2, dy + 1, dz + 1, ox.aE.bO, 10);
        }
        else if (f == 2) {
            this.worldObj.d(dx + 1, dy + 0, dz + 2, ox.aE.bO, 3);
            this.worldObj.d(dx + 1, dy + 1, dz + 2, ox.aE.bO, 11);
        }
        else if (f == 3) {
            this.worldObj.d(dx + 0, dy + 0, dz + 1, ox.aE.bO, 0);
            this.worldObj.d(dx + 0, dy + 1, dz + 1, ox.aE.bO, 8);
        }
    }
    ...
    void deadEndDoorSteel(final int x, final int z, final int f) {
        ...
        this.deadEndDoorway(x, z, f);
        if (f == 0) {
            this.worldObj.d(dx + 1, dy + 0, dz + 0, ox.aL.bO, 1);
            this.worldObj.d(dx + 1, dy + 1, dz + 0, ox.aL.bO, 9);
        }
        else if (f == 1) {
            this.worldObj.d(dx + 2, dy + 0, dz + 1, ox.aL.bO, 2);
            this.worldObj.d(dx + 2, dy + 1, dz + 1, ox.aL.bO, 10);
        }
        else if (f == 2) {
            this.worldObj.d(dx + 1, dy + 0, dz + 2, ox.aL.bO, 3);
            this.worldObj.d(dx + 1, dy + 1, dz + 2, ox.aL.bO, 11);
        }
        else if (f == 3) {
            this.worldObj.d(dx + 0, dy + 0, dz + 1, ox.aL.bO, 0);
            this.worldObj.d(dx + 0, dy + 1, dz + 1, ox.aL.bO, 8);
        }
    }
    ...
    protected boolean placeMobSpawner(final int dx, final int dy, final int dz, final String mobID) {
        this.worldObj.g(dx, dy, dz, ox.as.bO);
        final ch ms = (ch)this.worldObj.b(dx, dy, dz);
        if (ms != null) {
            ms.a(mobID);
            return true;
        }
    ...
    private boolean roomSpiderweb(final int dx, final int dy, final int dz, final int diameter) {
        ...
        this.worldObj.g(rx, dy + 2, rz, ox.W.bO);
        return true;
    }
}
