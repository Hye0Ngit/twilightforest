...
public class TFGenCanopyMushroom extends TFGenerator
{
    ...
    @Override
    public boolean a(final wz world, final Random random, final int treeX, final int treeY, final int treeZ) {
        this.worldObj = world;
        this.treeRNG = random;
        final int blockUnder = world.a(treeX, treeY - 1, treeZ);
        if ((blockUnder != ox.u.bO && blockUnder != ox.v.bO && blockUnder != ox.by.bO) || this.y >= 128 - this.treeHeight - 1) {
            return false;
        }
        this.x = treeX;
        this.y = treeY;
        this.z = treeZ;
        this.treeBlock = ox.bn.bO;
        this.treeMeta = 10;
        this.leafBlock = ((this.treeRNG.nextInt(3) == 0) ? ox.bo.bO : ox.bn.bO);
        this.leafMeta = 5;
        this.treeHeight = 12;
    ...
    void buildBranch(final int height, final double length, final double angle, final double tilt, final boolean firefly) {
        ...
        this.drawBresehnam(src[0], src[1], src[2], dest[0], src[1], dest[2], this.treeBlock, this.treeMeta, true);
        this.drawBresehnam(dest[0], src[1], dest[2], dest[0], dest[1] - 1, dest[2], this.treeBlock, this.treeMeta, true);
        if (firefly) {
