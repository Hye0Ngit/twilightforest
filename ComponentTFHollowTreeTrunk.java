...
public class ComponentTFHollowTreeTrunk extends nk
{
    int radius;
    int height;
    
    public ComponentTFHollowTreeTrunk(final ry world, final Random rand, final int index, final int x, final int y, final int z) {
        super(index);
        this.height = rand.nextInt(64) + 32;
        this.radius = rand.nextInt(4) + 1;
        this.f = 0;
        this.e = new nl(x, y, z, x + this.radius * 2, y + this.height, z + this.radius * 2);
    }
            ...
            for (int dz = 0; dz <= 2 * this.radius; ++dz) {
                for (int dy = 0; dy <= this.height; ++dy) {
                    ...
                    if (dist <= this.radius && dist > hollow) {
                        this.a(world, TFBlocks.wood.bM, 0, dx, dy, dz, sbb);
                    }
                    if (dist == hollow && dx == hollow + this.radius) {
                        this.a(world, yy.aF.bM, 4, dx, dy, dz, sbb);
                    }
                }
            }
        }
