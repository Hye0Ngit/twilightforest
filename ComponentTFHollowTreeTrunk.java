...
public class ComponentTFHollowTreeTrunk extends hb
{
    int radius;
    int height;
    
    public ComponentTFHollowTreeTrunk(final wz world, final Random rand, final int index, final int x, final int y, final int z) {
        super(index);
        this.height = rand.nextInt(64) + 32;
        this.radius = rand.nextInt(4) + 1;
        this.h = 0;
        this.g = new qc(x, y, z, x + this.radius * 2, y + this.height, z + this.radius * 2);
    }
    
    public void a(final hb structurecomponent, final List list, final Random rand) {
        final int index = this.c();
        final ComponentTFLeafSphere leafBlob = new ComponentTFLeafSphere(index + 1, this.g.d, this.g.e, this.g.f, 3);
        list.add(leafBlob);
    ...
    public void makeSmallBranch(final List list, final Random rand, final int index, final int branchHeight, final int branchLength, final double branchRotation, final double branchAngle, final boolean leafy) {
        final int[] bSrc = TFGenerator.translate(this.g.a + this.radius, this.g.b + branchHeight, this.g.c + this.radius, this.radius, branchRotation, 0.5);
            ...
            for (int dz = 0; dz <= 2 * this.radius; ++dz) {
                for (int dy = 0; dy <= this.height; ++dy) {
                    ...
                    if (dist <= this.radius && dist > hollow) {
                        this.a(world, TFBlocks.wood.bO, 0, dx, dy, dz, sbb);
                    }
                    if (dist == hollow && dx == hollow + this.radius) {
                        this.a(world, ox.aF.bO, 4, dx, dy, dz, sbb);
                    }
                }
            }
        }
