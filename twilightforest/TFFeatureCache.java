...
{
    private final aau chunkManager;
    private int lastSavedSize;
    public static final String fileName = "twilight.fc";
    private kv cacheMap;
    private ArrayList cache;
    
    public TFFeatureCache(final aau par1WorldChunkManager) {
        this.lastSavedSize = 0;
        this.cacheMap = new kv();
    ...
    static aau getChunkManager(final TFFeatureCache par0BiomeCache) {
    ...
    public void load(final File saveDir) {
        ...
        if (this.cache.size() == this.cacheMap.a()) {
            ...
            FMLLog.warning("[TwilightForest] Feature cache mismatch, cache.size=%d, map.elements=%d.", new Object[] { this.cache.size(), this.cacheMap.a() });
        }
    }
}
