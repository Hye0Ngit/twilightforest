...
public class TFTeleporter extends aai
{
    protected iz myWorld;
    protected Random rand;
    
    public TFTeleporter(final iz par1WorldServer) {
    ...
    public boolean b(final mp entity, final double par3, final double par5, final double par7, final float par9) {
        final int c = 200;
        double d = -1.0;
        int i = 0;
        int j = 0;
        int k = 0;
        final int l = kx.c(entity.u);
        final int i2 = kx.c(entity.w);
        for (int j2 = l - c; j2 <= l + c; ++j2) {
            ...
            for (int j3 = i2 - c; j3 <= i2 + c; ++j3) {
                ...
                for (int k2 = TFWorld.WORLDHEIGHT - 1; k2 >= 0; --k2) {
                    if (this.isBlockPortal((zv)this.myWorld, j2, k2, j3)) {
                        while (this.isBlockPortal((zv)this.myWorld, j2, k2 - 1, j3)) {
                            --k2;
                        }
        ...
        if (d >= 0.0) {
            ...
            if (this.isBlockPortal((zv)this.myWorld, k3 - 1, l2, i3)) {
                portalX -= 0.5;
            }
            if (this.isBlockPortal((zv)this.myWorld, k3 + 1, l2, i3)) {
                portalX += 0.5;
            }
            if (this.isBlockPortal((zv)this.myWorld, k3, l2, i3 - 1)) {
                portalZ -= 0.5;
            }
            if (this.isBlockPortal((zv)this.myWorld, k3, l2, i3 + 1)) {
                portalZ += 0.5;
    ...
    public boolean isBlockPortal(final zv world, final int x, final int y, final int z) {
        return world.a(x, y, z) == TFBlocks.portal.cz;
    }
    
    public boolean a(final mp entity) {
        final double yFactor = (this.myWorld.t.h == 0) ? 2.0 : 0.5;
        final byte byte0 = 16;
        double weight = -1.0;
        final int ex = kx.c(entity.u);
        final int ey = kx.c(entity.v * yFactor);
        final int ez = kx.c(entity.w);
        int px = ex;
        int py = ey;
        int pz = ez;
        int k1 = 0;
        final int l1 = this.myWorld.s.nextInt(4);
        for (int i2 = ex - byte0; i2 <= ex + byte0; ++i2) {
            final double d1 = i2 + 0.5 - entity.u;
            for (int j3 = ez - byte0; j3 <= ez + byte0; ++j3) {
                final double d2 = j3 + 0.5 - entity.w;
            Label_0465:
                for (int k2 = 127; k2 >= 0; --k2) {
                    if (this.myWorld.c(i2, k2, j3)) {
                        ...
                        for (int k3 = l1; k3 < l1 + 4; ++k3) {
                            ...
                            final double d3 = k2 + 0.5 - entity.v * yFactor;
        ...
        if (weight < 0.0) {
            for (int j7 = ex - byte0; j7 <= ex + byte0; ++j7) {
                final double d5 = j7 + 0.5 - entity.u;
                for (int k5 = ez - byte0; k5 <= ez + byte0; ++k5) {
                    final double d6 = k5 + 0.5 - entity.w;
                Label_0821:
                    for (int l5 = 127; l5 >= 0; --l5) {
                        if (this.myWorld.c(j7, l5, k5)) {
                            ...
                            for (int l6 = l1; l6 < l1 + 2; ++l6) {
                                ...
                                final double d7 = l5 + 0.5 - entity.v * yFactor;
        ...
        this.makePortalAt((zv)this.myWorld, portalX, portalY, portalZ);
        return true;
    }
    
    private void makePortalAt(final zv world, final int px, int py, final int pz) {
        if (py < 30) {
            py = 30;
        }
        world.getClass();
        ...
        --py;
        world.f(px - 1, py + 0, pz - 1, aou.y.cz, 0, 2);
        world.f(px + 0, py + 0, pz - 1, aou.y.cz, 0, 2);
        world.f(px + 1, py + 0, pz - 1, aou.y.cz, 0, 2);
        world.f(px + 2, py + 0, pz - 1, aou.y.cz, 0, 2);
        world.f(px - 1, py + 0, pz + 0, aou.y.cz, 0, 2);
        world.f(px + 2, py + 0, pz + 0, aou.y.cz, 0, 2);
        world.f(px - 1, py + 0, pz + 1, aou.y.cz, 0, 2);
        world.f(px + 2, py + 0, pz + 1, aou.y.cz, 0, 2);
        world.f(px - 1, py + 0, pz + 2, aou.y.cz, 0, 2);
        world.f(px + 0, py + 0, pz + 2, aou.y.cz, 0, 2);
        world.f(px + 1, py + 0, pz + 2, aou.y.cz, 0, 2);
        world.f(px + 2, py + 0, pz + 2, aou.y.cz, 0, 2);
        world.f(px + 0, py - 1, pz + 0, aou.z.cz, 0, 2);
        world.f(px + 1, py - 1, pz + 0, aou.z.cz, 0, 2);
        world.f(px + 0, py - 1, pz + 1, aou.z.cz, 0, 2);
        world.f(px + 1, py - 1, pz + 1, aou.z.cz, 0, 2);
        world.f(px + 0, py + 0, pz + 0, TFBlocks.portal.cz, 0, 2);
        world.f(px + 1, py + 0, pz + 0, TFBlocks.portal.cz, 0, 2);
        world.f(px + 0, py + 0, pz + 1, TFBlocks.portal.cz, 0, 2);
        world.f(px + 1, py + 0, pz + 1, TFBlocks.portal.cz, 0, 2);
    ...
    public int randNatureBlockID(final Random random) {
        final int[] block = { aou.aj.cz, aou.ak.cz, aou.ab.cz, aou.ai.cz, aou.ah.cz };
