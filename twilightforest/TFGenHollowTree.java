...
public class TFGenHollowTree extends TFGenerator
{
    ...
    public boolean a(final xd world, final Random random, final int treeX, final int treeY, final int treeZ) {
        ...
        for (int crownRadius = this.diameter * 4 + 8, dx = -crownRadius; dx <= crownRadius; ++dx) {
            for (int dz = -crownRadius; dz <= crownRadius; ++dz) {
                for (int dy = this.height - crownRadius; dy <= this.height + crownRadius; ++dy) {
                    ...
                    if (whatsThere != 0 && whatsThere != pb.K.bO) {
                        return false;
                    }
                }
            }
        ...
        this.buildBranchRing(3, 2, 12, 0, 0.75, 0.0, 3, 5, 3, false);
            ...
            for (int dz = -this.diameter; dz <= this.diameter; ++dz) {
                for (int dy = -4; dy < 0; ++dy) {
                    ...
                    if (dist <= this.diameter) {
                        if (this.hasAirAround((ali)this.worldObj, dx + this.x, dy + this.y, dz + this.z)) {
                            this.worldObj.b(dx + this.x, dy + this.y, dz + this.z, this.treeBlock, this.treeMeta);
                        }
                        else {
                            this.worldObj.b(dx + this.x, dy + this.y, dz + this.z, this.rootBlock, this.rootMeta);
                        }
                    }
                }
            }
            ...
            for (int dz = -this.diameter; dz <= this.diameter; ++dz) {
                for (int dy = 0; dy <= this.height; ++dy) {
                    ...
                    if (dist == hollow && dx == hollow) {
                        this.putBlockAndMetadata(dx + this.x, dy + this.y, dz + this.z, pb.bu.bO, 8, true);
                    }
                }
            }
        }
    ...
    private void makeRoot(final int branchHeight, final double length, final double angle, final double tilt) {
        ...
        final int[] dest = TFGenerator.translate(src[0], src[1], src[2], length, angle, tilt);
        final int[] lineArray = this.getBresehnamArray(src[0], src[1], src[2], dest[0], dest[1], dest[2]);
        boolean stillAboveGround = true;
        for (int i = 0; i < lineArray.length; i += 3) {
            if (stillAboveGround && this.hasAirAround((ali)this.worldObj, lineArray[i + 0], lineArray[i + 1], lineArray[i + 2])) {
                this.worldObj.b(lineArray[i + 0], lineArray[i + 1], lineArray[i + 2], this.treeBlock, this.treeMeta);
                this.worldObj.b(lineArray[i + 0], lineArray[i + 1] - 1, lineArray[i + 2], this.treeBlock, this.treeMeta);
            }
            else {
                this.worldObj.b(lineArray[i + 0], lineArray[i + 1], lineArray[i + 2], this.rootBlock, this.rootMeta);
                this.worldObj.b(lineArray[i + 0], lineArray[i + 1] - 1, lineArray[i + 2], this.rootBlock, this.rootMeta);
    ...
    private void addFirefly(final int fHeight, double fAngle) {
        ...
        fAngle %= 1.0;
        final int tmeta = 0;
        if (TFBlocks.critter.e(this.worldObj, src[0], src[1], src[2])) {
            this.putBlockAndMetadata(src[0], src[1], src[2], TFBlocks.critter.bO, tmeta, false);
        }
    }
