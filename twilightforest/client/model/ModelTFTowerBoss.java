...
{
    protected bcr[][] subTentacles;
    protected bcr[][] smallTentacles;
    
    public ModelTFTowerBoss() {
        this.smallTentacles = new bcr[2][3];
    ...
    protected void makeTentacle(final byte yOffset, final Random random, final int num) {
        this.tentacles[num] = new bcr((bbl)this, num % 3, 0);
        int length = 5;
        ...
        if (this.subTentacles == null) {
            this.subTentacles = new bcr[this.tentacles.length][3];
        }
        for (int i = 0; i < 3; ++i) {
            length = 4;
            (this.subTentacles[num][i] = new bcr((bbl)this, num % 4, i * 5 - 1)).a(-1.5f, -0.5f, -1.5f, 3, length, 3);
    ...
    @Override
    public void a(final float par1, final float par2, final float timeAlive, final float yaw, final float pitch, final float par6, final nm par7Entity) {
        ...
        for (int i = 0; i < this.subTentacles.length; ++i) {
            ...
            this.subTentacles[i][0].f = (lr.b(time * 0.6662f) * 1.0f - 1.0471976f) * wiggle;
            this.subTentacles[i][1].f = lr.b(time * 0.7774f) * 1.2f * wiggle;
            this.subTentacles[i][2].f = lr.b(time * 0.8886f + 1.5707964f) * 1.4f * wiggle;
            this.subTentacles[i][0].f = 0.2f + lr.b(time * 0.3335f) * 0.15f;
            this.subTentacles[i][1].f = 0.1f + lr.b(time * 0.4445f) * 0.2f;
            this.subTentacles[i][2].f = 0.1f + lr.b(time * 0.5555f) * 0.25f;
            final float yTwist = 0.4f;
            this.tentacles[i].g = yTwist * lr.a(time * 0.3f);
        }
    }
}
