...
public class TFMagicMapRenderer implements IItemRenderer
{
    ...
    private asl gameSettings;
    private atj fontRenderer;
    
    public TFMagicMapRenderer(final atj par1FontRenderer, final asl par2GameSettings, final bap par3RenderEngine) {
    ...
    public boolean handleRenderType(final um item, final IItemRenderer.ItemRenderType type) {
        return item.c == TFItems.magicMap.cg && type == IItemRenderer.ItemRenderType.FIRST_PERSON_MAP;
    }
    
    public boolean shouldUseRenderHelper(final IItemRenderer.ItemRenderType type, final um item, final IItemRenderer.ItemRendererHelper helper) {
        return false;
    }
    
    public void renderItem(final IItemRenderer.ItemRenderType type, final um item, final Object... data) {
        this.renderMap((qx)data[0], (bap)data[1], (ahg)data[2]);
    }
    
    public void renderMap(final qx par1EntityPlayer, final bap par2RenderEngine, final ahg par3MapData) {
        ...
        for (final ahi mapCoord : par3MapData.g.values()) {
            GL11.glPushMatrix();
        ...
        for (final ahi mapCoord : magicMapData.featuresVisibleOnMap) {
            GL11.glPushMatrix();
