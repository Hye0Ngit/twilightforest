...
public class TFClientTicker implements IScheduledTickHandler
{
    ...
    public void tickStart(final EnumSet type, final Object... tickData) {
        ...
        if (type.contains(TickType.CLIENT)) {
            final xv world = (xv)this.mc.e;
            final qx player = (qx)this.mc.g;
            if (world != null && world.v instanceof WorldProviderTwilightForest) {
                if (world.v.getSkyProvider() == null) {
                    world.v.setSkyProvider((SkyProvider)new TFSkyProvider());
                }
                this.mc.v.a = 0.0f;
            }
            synchronized (this) {
                for (final aze particle : this.particleBuffer) {
    ...
    public void addParticle(final aze particle) {
        synchronized (this) {
