...
public class EntityTFHydra extends md implements pl, pm
{
    ...
    public EntityTFHydra(final xv world) {
        super(world);
        this.numHeads = 7;
    ...
    public EntityTFHydra(final xv world, final double x, final double y, final double z) {
        this(world);
        this.b(x, y, z);
    }
    ...
    public void c() {
        if ((this.hc[0].headEntity == null || this.hc[1].headEntity == null || this.hc[2].headEntity == null) && this.shouldSpawnHeads() && !this.p.J) {
            for (int i = 0; i < this.numHeads; ++i) {
        ...
        if (this.aU > 0) {
            for (int i = 0; i < this.numHeads; ++i) {
                this.hc[i].setHurtTime(this.aU);
            }
        }
        ++this.ticksSinceDamaged;
        if (!this.p.J && this.ticksSinceDamaged > EntityTFHydra.TICKS_BEFORE_HEALING && this.ticksSinceDamaged % 5 == 0) {
            this.i(1);
        }
        ...
        this.tail.N = 6.0f;
        this.tail.O = 2.0f;
        final float angle = (this.aw + 180.0f) * 3.141593f / 180.0f;
    ...
    protected void bn() {
        ...
        if (this.aa.nextFloat() < 0.7f) {
            ...
            if (entityplayer2 != null) {
                ...
                this.bH = 100 + this.aa.nextInt(20);
            }
            else {
        ...
        if (this.currentTarget != null) {
            ...
            if (this.bH-- <= 0 || this.currentTarget.L || this.currentTarget.e((lq)this) > f * f) {
                this.currentTarget = null;
            }
        }
        else {
    ...
    public md findSecondaryTarget(final double range) {
        ...
        final List nearbyEntities = this.p.a((Class)md.class, anw.a().a(this.t, this.u, this.v, this.t + 1.0, this.u + 1.0, this.v + 1.0).b(range, range, range));
    ...
    private boolean destroyBlocksInAABB(final anw par1AxisAlignedBB) {
            ...
            for (int dy = minY; dy <= maxY; ++dy) {
                for (int dz = minZ; dz <= maxZ; ++dz) {
                    ...
                    if (currentID != 0) {
                        ...
                        if (currentID != amj.as.cm && currentID != amj.bM.cm && currentID != amj.C.cm) {
                            didDestroy = true;
                            this.p.e(dx, dy, dz, 0);
    ...
    public boolean attackEntityFromPart(final EntityTFHydraPart part, final lh damagesource, final int damageAmount) {
        if (!this.p.J && damagesource == lh.d && part.E() != null) {
    ...
    protected void a(final boolean par1, final int par2) {
        for (int totalDrops = this.aa.nextInt(3 + par2) + 5, i = 0; i < totalDrops; ++i) {
            this.b(TFItems.hydraChop.cg, 5);
        }
        for (int totalDrops = this.aa.nextInt(4 + par2) + 7, i = 0; i < totalDrops; ++i) {
            this.b(TFItems.fieryBlood.cg, 1);
        }
        this.b(TFItems.hydraTrophy.cg, 1);
    }
    ...
    protected void aP() {
        ...
        if (this.aX <= 140 && this.aX % 20 == 0) {
            final int headToDie = this.aX / 20 - 1;
