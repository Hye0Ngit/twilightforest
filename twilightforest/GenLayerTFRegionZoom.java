...
public class GenLayerTFRegionZoom extends wp
{
    public boolean shiftCenter;
    
    public GenLayerTFRegionZoom(final long l, final wp genlayer, final boolean sc) {
        super(l);
        super.a = genlayer;
        this.shiftCenter = sc;
    }
    ...
    public int[] a(final int x, final int z, final int width, final int depth) {
        ...
        final int[] dest = ad.a(swidth * 2 * (sdepth * 2));
        ...
        for (int dz = 0; dz < sdepth - 1; ++dz) {
            ...
            for (int dx = 0; dx < swidth - 1; ++dx) {
                ...
                if (this.shiftCenter) {
                    dest[di] = (TFBiomeBase.isFeature(tl) ? br : tl);
                }
                else {
                    dest[di] = tl;
                }
    ...
    protected int chooseRandom(final int i, final int j) {
        if (TFBiomeBase.isFeature(i)) {
            return j;
        }
        if (TFBiomeBase.isFeature(j)) {
            return i;
        }
    ...
    protected int chooseWeighted(final int tl, final int tr, final int bl, final int br) {
        if (this.shiftCenter && TFBiomeBase.isFeature(tl)) {
            return tl;
        }
        if (tr == bl && bl == br) {
            return tr;
        ...
        if (TFBiomeBase.isFeature(tl)) {
            return this.chooseRandom(tr, bl, br);
        }
        if (TFBiomeBase.isFeature(tr)) {
            return this.chooseRandom(tl, bl, br);
        }
        if (TFBiomeBase.isFeature(bl)) {
            return this.chooseRandom(tl, tr, br);
        }
        if (TFBiomeBase.isFeature(br)) {
    ...
    public static wp multipleZoom(final long l, final wp genlayer, final int i) {
        Object obj = genlayer;
        for (int j = 0; j < i; ++j) {
            obj = new GenLayerTFRegionZoom(l + j, (wp)obj, false);
        }
        return (wp)obj;
    }
}
