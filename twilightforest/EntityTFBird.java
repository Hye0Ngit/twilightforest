// 
// Decompiled by Procyon v0.6-prerelease
// 

package twilightforest;

public abstract class EntityTFBird extends bc
{
    public float flapLength;
    public float flapIntensity;
    public float lastFlapIntensity;
    public float lastFlapLength;
    public float flapSpeed;
    
    public EntityTFBird(final xd par1World) {
        super(par1World);
        this.flapLength = 0.0f;
        this.flapIntensity = 0.0f;
        this.flapSpeed = 1.0f;
    }
    
    public boolean b_() {
        return true;
    }
    
    public void e() {
        super.e();
        this.lastFlapLength = this.flapLength;
        this.lastFlapIntensity = this.flapIntensity;
        this.flapIntensity += (float)((this.z ? -1 : 4) * 0.3);
        if (this.flapIntensity < 0.0f) {
            this.flapIntensity = 0.0f;
        }
        if (this.flapIntensity > 1.0f) {
            this.flapIntensity = 1.0f;
        }
        if (!this.z && this.flapSpeed < 1.0f) {
            this.flapSpeed = 1.0f;
        }
        this.flapSpeed *= (float)0.9;
        if (!this.z && this.s < 0.0) {
            this.s *= 0.6;
        }
        this.flapLength += this.flapSpeed * 2.0f;
    }
    
    protected void e(final float par1) {
    }
    
    protected boolean e_() {
        return false;
    }
    
    protected int f() {
        return yr.L.bQ;
    }
    
    public bc a(final bc var1) {
        return null;
    }
}
