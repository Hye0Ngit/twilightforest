...
{
    public EntityTFMinotaur(final xv par1World) {
        super(par1World);
        this.aF = "/twilightforest/minotaur.png";
        this.bG = 0.25f;
    ...
    protected void a(final boolean par1, final int par2) {
        for (int numDrops = this.aa.nextInt(2) + this.aa.nextInt(1 + par2), i = 0; i < numDrops; ++i) {
            if (this.af()) {
                this.b(TFItems.meefSteak.cg, 1);
            }
            else {
                this.b(TFItems.meefRaw.cg, 1);
            }
        }
    }
    
    protected void l(final int par1) {
        this.b(TFItems.mazeMapFocus.cg, 1);
    }
