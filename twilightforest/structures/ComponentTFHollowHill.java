...
public class ComponentTFHollowHill extends StructureTFComponent
{
    ...
    public ComponentTFHollowHill(final abv world, final Random rand, final int i, final int size, final int x, final int y, final int z) {
        super(i);
        this.setCoordBaseMode(0);
        this.hillSize = size;
        this.radius = (this.hillSize * 2 + 1) * 8 - 6;
        this.f = StructureTFComponent.getComponentToAddBoundingBox(x, y, z, -this.radius, -3, -this.radius, this.radius * 2, 10, this.radius * 2, 0);
    }
    ...
    protected void generateTreasureChest(final abv world, final int x, final int y, final int z, final age sbb) {
        final Random chestRNG = new Random(world.H() + x * z);
        ...
        this.b(world, aqw.B.cF, 0, x, y - 1, z, sbb);
    }
    
    protected void generateOreStalactite(final abv world, final int x, final int y, final int z, final age sbb) {
        final int dx = this.a(x, z);
        final int dy = this.a(y);
        final int dz = this.b(x, z);
        if (sbb.b(dx, dy, dz) && world.a(dx, dy, dz) != aqw.ax.cF) {
            final Random stalRNG = new Random(world.H() + dx * dz);
    ...
    protected void generateStoneStalactite(final abv world, float length, final boolean up, final int x, final int y, final int z, final age sbb) {
        final int dx = this.a(x, z);
        final int dy = this.a(y);
        final int dz = this.b(x, z);
        if (sbb.b(dx, dy, dz) && world.a(dx, dy, dz) != aqw.ax.cF) {
            ...
            new TFGenCaveStalactite(aqw.y.cF, length, up).a(world, stalRNG, dx, dy, dz);
        }
    }
    ...
    boolean isInHill(final int mapX, final int mapY, final int mapZ) {
        final int dx = this.f.a + this.radius - mapX;
        final int dy = (this.f.b - mapY) * 2;
        final int dz = this.f.c + this.radius - mapZ;
