...
public class ComponentTFMinotaurMaze extends StructureTFComponent
{
    ...
    public ComponentTFMinotaurMaze(final int index, final int x, final int y, final int z, final int entranceX, final int entranceZ, final int level) {
        ...
        this.level = level;
        this.f = StructureTFComponent.getComponentToAddBoundingBox(x, y, z, -this.getRadius(), 0, -this.getRadius(), this.getRadius() * 2, 5, this.getRadius() * 2, 0);
        (this.maze = new TFMaze(this.getMazeSize(), this.getMazeSize())).setSeed(this.f.a * 90342903 + this.f.b * 90342903 ^ this.f.c);
        final int nrooms = 7;
    ...
    protected ComponentTFMazeRoom makeRoom(final Random random, final int i, final int dx, final int dz) {
        ...
        final int worldX = this.f.a + dx * 5 - 4;
        final int worldY = this.f.b;
        final int worldZ = this.f.c + dz * 5 - 4;
        if (i == 0) {
    ...
    protected void decorateDeadEndsCorridors(final Random random, final List list) {
        for (int x = 0; x < this.maze.width; ++x) {
            for (int z = 0; z < this.maze.depth; ++z) {
                ...
                if (component != null) {
                    list.add(component);
                    component.a((aiq)this, list, random);
                }
            }
        }
    }
    ...
    protected ComponentTFMazeDeadEnd makeDeadEnd(final Random random, final int dx, final int dz, final int rotation) {
        final int worldX = this.f.a + dx * 5 + 1;
        final int worldY = this.f.b;
        final int worldZ = this.f.c + dz * 5 + 1;
    ...
    public void a(final aiq structurecomponent, final List list, final Random random) {
        ...
        if (this.level == 1) {
            final int centerX = this.f.a + (this.f.d - this.f.a) / 2;
            final int centerZ = this.f.c + (this.f.f - this.f.c) / 2;
            final ComponentTFMinotaurMaze maze = new ComponentTFMinotaurMaze(1, centerX, this.f.b - 10, centerZ, this.rcoords[2], this.rcoords[3], 2);
            list.add(maze);
