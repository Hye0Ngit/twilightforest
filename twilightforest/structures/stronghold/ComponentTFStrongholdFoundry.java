...
public class ComponentTFStrongholdFoundry extends StructureTFStrongholdComponent
{
        ...
        for (int x = 4; x < 14; ++x) {
            for (int z = 4; z < 14; ++z) {
                for (int y = 8; y < 23; ++y) {
                    ...
                    if (c < r) {
                        this.func_151550_a(world, Blocks.field_150348_b, 0, x, y, z, sbb);
                    }
                }
            }
        }
        for (int i = 0; i < 400; ++i) {
            ...
            if (this.func_151548_a(world, dx, dy, dz, sbb) != Blocks.field_150350_a) {
                for (int y2 = 0; y2 < 3; ++y2) {
                    this.func_151550_a(world, Blocks.field_150348_b, 0, dx, dy - y2, dz, sbb);
                }
            }
        }
        for (int i = 0; i < 8; ++i) {
            final Block blockID = Blocks.field_150450_ax;
            final int blockMeta = 0;
            this.addOreToMass(world, sbb, massRandom, blockID, blockMeta);
        }
        for (int i = 0; i < 8; ++i) {
            final Block blockID = Blocks.field_150366_p;
            final int blockMeta = 0;
            this.addOreToMass(world, sbb, massRandom, blockID, blockMeta);
        }
        for (int i = 0; i < 6; ++i) {
            final Block blockID = Blocks.field_150352_o;
            final int blockMeta = 0;
            this.addOreToMass(world, sbb, massRandom, blockID, blockMeta);
        }
        for (int i = 0; i < 2; ++i) {
            final Block blockID = Blocks.field_150426_aN;
            final int blockMeta = 0;
            this.addOreToMass(world, sbb, massRandom, blockID, blockMeta);
        }
        for (int i = 0; i < 2; ++i) {
            final Block blockID = Blocks.field_150412_bA;
            final int blockMeta = 0;
            this.addOreToMass(world, sbb, massRandom, blockID, blockMeta);
        }
        for (int i = 0; i < 4; ++i) {
            final Block blockID = Blocks.field_150482_ag;
            final int blockMeta = 0;
            this.addOreToMass(world, sbb, massRandom, blockID, blockMeta);
        }
    ...
    private void addOreToMass(final World world, final StructureBoundingBox sbb, final Random massRandom, final Block blockID, final int blockMeta) {
        for (int i = 0; i < 10; ++i) {
            final int dx = massRandom.nextInt(9) + 5;
            final int dz = massRandom.nextInt(9) + 5;
            final int dy = massRandom.nextInt(13) + 10;
