...
public class ComponentTFLeafSphere extends StructureTFComponent
{
    ...
    protected ComponentTFLeafSphere(final int index, final int x, final int y, final int z, final int radius) {
        ...
        this.e = new aee(x - radius, y - radius, z - radius, x + radius, y + radius, z + radius);
        this.rad = radius;
    }
        ...
        for (byte dx = 0; dx <= this.rad; ++dx) {
            for (byte dy = 0; dy <= this.rad; ++dy) {
                for (byte dz = 0; dz <= this.rad; ++dz) {
                    ...
                    if (dist <= this.rad) {
                        this.placeBlockIfEmpty(world, TFBlocks.leaves.cz, 0, sx + dx, sy + dy, sz + dz, sbb);
                        this.placeBlockIfEmpty(world, TFBlocks.leaves.cz, 0, sx + dx, sy + dy, sz - dz, sbb);
                        this.placeBlockIfEmpty(world, TFBlocks.leaves.cz, 0, sx - dx, sy + dy, sz + dz, sbb);
                        this.placeBlockIfEmpty(world, TFBlocks.leaves.cz, 0, sx - dx, sy + dy, sz - dz, sbb);
                        this.placeBlockIfEmpty(world, TFBlocks.leaves.cz, 0, sx + dx, sy - dy, sz + dz, sbb);
    ...
    protected void placeBlockIfEmpty(final zv world, final int blockID, final int meta, final int x, final int y, final int z, final aee sbb) {
