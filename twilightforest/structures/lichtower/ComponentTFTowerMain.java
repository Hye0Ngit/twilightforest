...
public class ComponentTFTowerMain extends ComponentTFTowerWing
{
    ...
    public boolean makeTowerOutbuilding(final List<StructureComponent> list, final Random rand, final int index, final int x, final int y, final int z, final int wingSize, final int wingHeight, final int rotation) {
    ...
    protected void makeStairCrossing(final World world, final Random rand, final int flight, final StructureBoundingBox sbb) {
        ...
        for (int dx = 6; dx <= 8; ++dx) {
            for (int dz = 4; dz <= 10; ++dz) {
                this.func_151550_a(world, (Block)Blocks.field_150334_T, floorMeta, dx, floorLevel, dz, sbb);
            }
        }
        ++floorLevel;
        int dx = 6;
        for (int dz = 3; dz <= 11; ++dz) {
            this.func_151550_a(world, Blocks.field_150422_aJ, 0, dx, floorLevel, dz, sbb);
        }
        ++dx;
        for (int dz = 3; dz <= 11; ++dz) {
            this.func_151550_a(world, Blocks.field_150350_a, 0, dx, floorLevel, dz, sbb);
        }
        ++dx;
        for (int dz = 3; dz <= 11; ++dz) {
            this.func_151550_a(world, Blocks.field_150422_aJ, 0, dx, floorLevel, dz, sbb);
        }
        this.func_151550_a(world, (Block)Blocks.field_150334_T, floorMeta, 6, floorLevel - 1, 11, sbb);
        this.func_151550_a(world, (Block)Blocks.field_150334_T, floorMeta, 8, floorLevel - 1, 3, sbb);
        ...
        for (int fx = 1; fx < 14; ++fx) {
            for (int fz = 1; fz < 14; ++fz) {
                if ((fx == 1 || fx == 2) && fz >= 6 && fz <= 12) {
                    if (fz == 6) {
                        this.func_151550_a(world, (Block)Blocks.field_150376_bx, 10, fx, floorLevel, fz, sbb);
                    }
                }
