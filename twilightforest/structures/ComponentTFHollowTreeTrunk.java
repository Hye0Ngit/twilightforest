...
public class ComponentTFHollowTreeTrunk extends StructureTFComponent
{
    ...
    public ComponentTFHollowTreeTrunk(final abv world, final Random rand, final int index, final int x, final int y, final int z) {
        super(index);
        this.height = rand.nextInt(64) + 32;
        this.radius = rand.nextInt(4) + 1;
        this.setCoordBaseMode(0);
        this.f = new age(x, y, z, x + this.radius * 2, y + this.height, z + this.radius * 2);
    }
    
    public void a(final aiq structurecomponent, final List list, final Random rand) {
        final int index = this.c();
        final ComponentTFLeafSphere leafBlob = new ComponentTFLeafSphere(index + 1, this.f.d, this.f.e, this.f.f, 3);
        list.add(leafBlob);
        leafBlob.a((aiq)this, list, rand);
    ...
    public void makeSmallBranch(final List list, final Random rand, final int index, final int branchHeight, final int branchLength, final double branchRotation, final double branchAngle, final boolean leafy) {
        final int[] bSrc = TFGenerator.translate(this.f.a + this.radius, this.f.b + branchHeight, this.f.c + this.radius, this.radius, branchRotation, 0.5);
            ...
            for (int dz = 0; dz <= 2 * this.radius; ++dz) {
                for (int dy = 0; dy <= this.height; ++dy) {
                    ...
                    if (dist <= this.radius && dist > hollow) {
                        this.a(world, TFBlocks.log.cF, 0, dx, dy, dz, sbb);
                    }
                    if (dist == hollow && dx == hollow + this.radius) {
                        this.a(world, aqw.aK.cF, 4, dx, dy, dz, sbb);
                    }
                }
            }
        }
