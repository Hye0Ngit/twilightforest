...
public class ComponentTFHollowTreeMedBranch extends StructureTFComponent
{
    ...
    public boolean func_74875_a(final World world, final Random random, final StructureBoundingBox sbb) {
        ...
        this.drawBresehnam(world, sbb, rSrc.field_71574_a, rSrc.field_71572_b + 1, rSrc.field_71573_c, rDest.field_71574_a, rDest.field_71572_b, rDest.field_71573_c, TFBlocks.log.field_71990_ca, 12);
    ...
    protected void drawBresehnam(final World world, final StructureBoundingBox sbb, final int x1, final int y1, final int z1, final int x2, final int y2, final int z2, final int blockValue, final int metaValue) {
        ...
        for (int dx = 0; dx <= radius; ++dx) {
            for (int dy = 0; dy <= radius; ++dy) {
                for (int dz = 0; dz <= radius; ++dz) {
                    ...
                    if (dist <= radius) {
                        this.placeLeafBlock(world, TFBlocks.leaves.field_71990_ca, 0, sx + dx, sy + dy, sz + dz, sbb);
                        this.placeLeafBlock(world, TFBlocks.leaves.field_71990_ca, 0, sx + dx, sy + dy, sz - dz, sbb);
                        this.placeLeafBlock(world, TFBlocks.leaves.field_71990_ca, 0, sx - dx, sy + dy, sz + dz, sbb);
                        this.placeLeafBlock(world, TFBlocks.leaves.field_71990_ca, 0, sx - dx, sy + dy, sz - dz, sbb);
                        this.placeLeafBlock(world, TFBlocks.leaves.field_71990_ca, 0, sx + dx, sy - dy, sz + dz, sbb);
    ...
    protected void placeLeafBlock(final World world, final int blockID, final int meta, final int x, final int y, final int z, final StructureBoundingBox sbb) {
        ...
        if (sbb.func_78890_b(offX, offY, offZ)) {
            final int whatsThere = world.func_72798_a(offX, offY, offZ);
            final Block block = Block.field_71973_m[whatsThere];
            if (block == null || block.canBeReplacedByLeaves(world, offX, offY, offZ)) {
                world.func_72832_d(offX, offY, offZ, blockID, meta, 2);
            }
        }
    }
    ...
    protected void drawSmallBranch(final World world, final StructureBoundingBox sbb, final int sx, final int sy, final int sz, final double branchLength, final double branchAngle, final double branchTilt, final boolean leafy) {
        ...
        this.drawBresehnam(world, sbb, sx, sy, sz, branchDest.field_71574_a, branchDest.field_71572_b, branchDest.field_71573_c, TFBlocks.log.field_71990_ca, 12);
