...
public class ComponentTFHollowTreeTrunk extends StructureTFComponent
{
            ...
            for (int dz = 0; dz <= 2 * this.radius; ++dz) {
                ...
                for (int dy = 0; dy <= this.height; ++dy) {
                    if (dist <= this.radius && dist > hollow) {
                        this.func_74864_a(world, TFBlocks.log.field_71990_ca, 0, dx + 1, dy, dz + 1, sbb);
                    }
                }
                if (dist <= this.radius) {
                    this.func_74870_b(world, TFBlocks.log.field_71990_ca, 0, dx + 1, -1, dz + 1, sbb);
                }
                if (dist == hollow && dx == hollow + this.radius) {
                    this.func_74870_b(world, Block.field_71998_bu.field_71990_ca, 8, dx + 1, this.height, dz + 1, sbb);
                }
            }
        }
    ...
    protected void addInsect(final World world, final int fHeight, double fAngle, final Random random, final StructureBoundingBox sbb) {
        ...
        this.addInsect(world, random.nextBoolean() ? TFBlocks.firefly.field_71990_ca : TFBlocks.cicada.field_71990_ca, insectMeta, bugSpot.field_71574_a, bugSpot.field_71572_b, bugSpot.field_71573_c, sbb);
    }
    
    private void addInsect(final World world, final int blockID, final int insectMeta, final int posX, final int posY, final int posZ, final StructureBoundingBox sbb) {
        final int ox = this.func_74865_a(posX, posZ);
        final int oy = this.func_74862_a(posY);
        final int oz = this.func_74873_b(posX, posZ);
        if (sbb.func_78890_b(ox, oy, oz) && Block.field_71973_m[blockID] != null && Block.field_71973_m[blockID].func_71930_b(world, ox, oy, oz)) {
            world.func_72832_d(ox, oy, oz, blockID, insectMeta, 2);
        }
    }
}
