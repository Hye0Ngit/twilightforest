...
public abstract class TFGenerator extends li
{
    protected xd worldObj;
    
    public abstract boolean a(final xd p0, final Random p1, final int p2, final int p3, final int p4);
    ...
    protected void drawBresehnamX(final int x1, final int y1, final int z1, final int x2, final int y2, final int z2, final int blockValue, final int metaValue, final boolean priority) {
    ...
    protected void drawBresehnam(final int x1, final int y1, final int z1, final int x2, final int y2, final int z2, final int blockValue, final int metaValue, final boolean priority) {
        final int[] lineArray = this.getBresehnamArray(x1, y1, z1, x2, y2, z2);
        for (int i = 0; i < lineArray.length; i += 3) {
            this.putBlockAndMetadata(lineArray[i + 0], lineArray[i + 1], lineArray[i + 2], blockValue, metaValue, priority);
        }
    }
    
    protected int[] getBresehnamArray(final int x1, final int y1, final int z1, final int x2, final int y2, final int z2) {
        final int[] pixel = { x1, y1, z1 };
        final int dx = x2 - x1;
        final int dy = y2 - y1;
        final int dz = z2 - z1;
    ...
    protected byte randStone(final Random rand, final int howMuch) {
        return (rand.nextInt(howMuch) >= 1) ? ((byte)pb.w.bO) : ((byte)pb.ao.bO);
    }
    
    protected boolean isAreaClear(final xd world, final Random rand, final int x, final int y, final int z, final int dx, final int dy, final int dz) {
        boolean flag = true;
        for (int cx = 0; cx < dx; ++cx) {
            for (int cz = 0; cz < dy; ++cz) {
                final acn m = world.f(x + cx, y - 1, z + cz);
                if (m != acn.c && m != acn.b && m != acn.e) {
                    flag = false;
                }
    ...
    public void drawBlob(final int sx, final int sy, final int sz, final int rad, final int blockValue, final int metaValue, final boolean priority) {
        for (byte dx = 0; dx <= rad; ++dx) {
            for (byte dy = 0; dy <= rad; ++dy) {
                for (byte dz = 0; dz <= rad; ++dz) {
                    ...
                    if (dist <= rad) {
                        this.putBlockAndMetadata(sx + dx, sy + dy, sz + dz, blockValue, metaValue, priority);
                        this.putBlockAndMetadata(sx + dx, sy + dy, sz - dz, blockValue, metaValue, priority);
                        this.putBlockAndMetadata(sx - dx, sy + dy, sz + dz, blockValue, metaValue, priority);
                        this.putBlockAndMetadata(sx - dx, sy + dy, sz - dz, blockValue, metaValue, priority);
                        this.putBlockAndMetadata(sx + dx, sy - dy, sz + dz, blockValue, metaValue, priority);
    ...
    protected boolean hasAirAround(final ali world, final int bx, final int by, final int bz) {
    ...
    protected boolean isNearSolid(final ali world, final int bx, final int by, final int bz) {
        boolean nearSolid = false;
        if (world.f(bx + 1, by, bz).a()) {
            nearSolid = true;
        }
        if (world.f(bx - 1, by, bz).a()) {
            nearSolid = true;
        }
        if (world.f(bx, by, bz + 1).a()) {
            nearSolid = true;
        }
        if (world.f(bx, by, bz - 1).a()) {
            nearSolid = true;
