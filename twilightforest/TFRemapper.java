...
public class TFRemapper
{
    ...
    public static void remapBlocks(final RegistryEvent.MissingMappings<Block> event) {
        for (final RegistryEvent.MissingMappings.Mapping<Block> mapping : event.getAllMappings()) {
            if (mapping.key.m_135827_().equals("twilightforest")) {
    ...
    @SubscribeEvent
    public static void remapEntities(final RegistryEvent.MissingMappings<EntityType<?>> event) {
        for (final RegistryEvent.MissingMappings.Mapping<EntityType<?>> mapping : event.getAllMappings()) {
            if (mapping.key.m_135827_().equals("twilightforest")) {
    ...
    private static void remapBlock(final RegistryEvent.MissingMappings.Mapping<Block> mapping, final String oldId, final String newId) {
        if (mapping.key.m_135815_().contains(oldId)) {
            final String newName = mapping.key.m_135815_().replace(oldId, newId);
            final ResourceLocation remap = new ResourceLocation("twilightforest", newName);
            mapping.remap((IForgeRegistryEntry)ForgeRegistries.BLOCKS.getValue(remap));
        }
    }
    
    private static void remapItem(final RegistryEvent.MissingMappings.Mapping<Item> mapping, final String oldId, final String newId) {
        if (mapping.key.m_135815_().contains(oldId)) {
            final String newName = mapping.key.m_135815_().replace(oldId, newId);
            final ResourceLocation remap = new ResourceLocation("twilightforest", newName);
            mapping.remap((IForgeRegistryEntry)ForgeRegistries.ITEMS.getValue(remap));
        }
    }
    
    private static void remapEntity(final RegistryEvent.MissingMappings.Mapping<EntityType<?>> mapping, final String oldId, final String newId) {
        if (mapping.key.m_135815_().contains(oldId)) {
            final String newName = mapping.key.m_135815_().replace(oldId, newId);
            final ResourceLocation remap = new ResourceLocation("twilightforest", newName);
            mapping.remap((IForgeRegistryEntry)ForgeRegistries.ENTITIES.getValue(remap));
        }
    }
}
