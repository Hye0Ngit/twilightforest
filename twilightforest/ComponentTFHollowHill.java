...
public class ComponentTFHollowHill extends StructureTFComponent
{
    ...
    public ComponentTFHollowHill(final xd world, final Random rand, final int i, final int size, final int x, final int y, final int z) {
        super(i);
    ...
    public void a(final he structurecomponent, final List list, final Random random) {
    ...
    private void generateTreasureChest(final xd world, final int x, final int y, final int z, final qg sbb) {
        final Random chestRNG = new Random(world.v() + x * z);
        this.placeTreasureAtCurrentPosition(world, chestRNG, x, y, z, TFTreasure.hill1, sbb);
        this.b(world, pb.w.bO, 0, x, y - 1, z, sbb);
    }
    
    protected void generateOreStalactite(final xd world, final int x, final int y, final int z, final qg sbb) {
        final int dx = this.a(x, z);
        final int dy = this.b(y);
        final int dz = this.b(x, z);
        if (sbb.b(dx, dy, dz) && world.a(dx, dy, dz) != pb.as.bO) {
            final Random stalRNG = new Random(world.v() + dx * dz);
    ...
    protected void generateStoneStalactite(final xd world, double length, final boolean up, final int x, final int y, final int z, final qg sbb) {
        final int dx = this.a(x, z);
        final int dy = this.b(y);
        final int dz = this.b(x, z);
        if (sbb.b(dx, dy, dz) && world.a(dx, dy, dz) != pb.as.bO) {
            ...
            new TFGenCaveStalactite(pb.t.bO, stalRNG.nextDouble() * length, up).a(world, stalRNG, dx, dy, dz);
        }
    }
