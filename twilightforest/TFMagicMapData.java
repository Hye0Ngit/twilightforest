...
public class TFMagicMapData extends ajf
{
    ...
    public void addFeatureToMap(final TFFeature feature, final int x, final int z) {
        ...
        if (relativeX >= -rangeX && relativeZ >= -rangeY && relativeX <= rangeX && relativeZ <= rangeY) {
            ...
            for (final ajh existingCoord : this.featuresVisibleOnMap) {
                if (existingCoord.b == mapX && existingCoord.c == mapZ) {
                    featureFound = true;
                }
            }
            if (!featureFound) {
                this.featuresVisibleOnMap.add(new ajh((ajf)this, markerIcon, mapX, mapZ, mapRotation));
            }
        }
    }
    
    public void checkExistingFeatures(final zv world) {
        ArrayList toRemove = null;
        for (final ajh coord : this.featuresVisibleOnMap) {
    ...
    public byte[] makeFeatureStorageArray() {
        ...
        for (int i = 0; i < this.featuresVisibleOnMap.size(); ++i) {
            final ajh featureCoord = this.featuresVisibleOnMap.get(i);
