...
public class EntityTFNagaSegment extends md implements qe
{
    ...
    public EntityTFNagaSegment(final yc world) {
        super(world);
        this.sDist = 1.5;
        this.aG = "/twilightforest/nagasegment.png";
        this.a(1.75f, 1.95f);
        this.X = 2.0f;
        this.aR = 1000;
        this.af = true;
    ...
    public EntityTFNagaSegment(final yc world, final double x, final double y, final double z) {
        this(world);
        this.b(x, y, z);
    }
    
    public EntityTFNagaSegment(final yc world, final EntityTFNaga head, final int segment) {
        this(world);
        this.head = head;
        this.setSegment(segment);
        this.bH = head.getMoveSpeed() * 1.5f;
        this.aN = head.aN;
        this.aO = head.aO;
    }
    ...
    protected void bn() {
        if (this.head != null && this.head.aZ <= 0) {
    ...
    public boolean a(final lh damagesource, final int i) {
        ...
        if (!this.p.I && this.deathCounter <= 0 && this.head != null) {
            final int n = 10;
            this.aW = n;
            this.aV = n;
    ...
    public void pullTowards(final lq leader) {
        this.bH = this.head.getMoveSpeed() * 1.5f;
        ...
        if (dist > this.sDist * 4.0) {
            ...
            this.bD = (float)(this.bH * dist * 5.0);
        }
        else if (dist > 0.0) {
            this.a(leader, 90.0f, 90.0f);
            this.bD = this.head.getMoveSpeed();
        }
        if (dy > 1.1) {
            this.bi();
        }
