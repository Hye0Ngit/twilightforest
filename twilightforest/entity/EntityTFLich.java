...
public class EntityTFLich extends qj implements pl
{
    ...
    private static final ur[] heldItems;
    ...
    public EntityTFLich(final yc world) {
        super(world);
        this.a(1.1f, 2.5f);
        this.aG = "/twilightforest/twilightlich64.png";
    ...
    public EntityTFLich(final yc world, final double x, final double y, final double z) {
        this(world);
        this.b(x, y, z);
    }
    
    public EntityTFLich(final yc world, final EntityTFLich otherLich) {
        this(world);
    ...
    private void dropScepter() {
        ...
        if (scepterType == 0) {
            this.a(new ur(TFItems.scepterZombie), 0.0f);
        }
        else if (scepterType == 1) {
            this.a(new ur(TFItems.scepterLifeDrain), 0.0f);
        }
        else {
            this.a(new ur(TFItems.scepterTwilight), 0.0f);
        }
    }
    ...
    private void dropGoldThing() {
        ...
        ur goldThing;
        if (thingType == 0) {
            goldThing = new ur(up.G);
        }
        else if (thingType == 1) {
            goldThing = new ur(up.al);
        }
        else if (thingType == 2) {
            goldThing = new ur(up.am);
        }
        else if (thingType == 3) {
            goldThing = new ur(up.an);
        }
        else {
            goldThing = new ur(up.ao);
        }
        xe.a(this.aa, goldThing, 10 + this.aa.nextInt(30));
    ...
    protected void a(final lq targetedEntity, final float f) {
        if (!this.isShadowClone() && this.aZ % 15 == 0) {
            this.popNearbyMob();
        }
        if (this.getPhase() == 1) {
            if (this.aZ == 60 && !this.p.I) {
                this.teleportToSightOfEntity(targetedEntity);
                if (!this.isShadowClone()) {
                    this.checkAndSpawnClones(targetedEntity);
                }
            }
            if (this.n(targetedEntity) && this.aZ == 0 && f < 20.0f) {
                if (this.getNextAttackType() == 0) {
        ...
        if (this.getPhase() == 3 && this.aZ <= 0 && f < 2.0f && targetedEntity.D.e > this.D.b && targetedEntity.D.b < this.D.e) {
            this.aZ = 20;
            this.m(targetedEntity);
            this.b = true;
        }
    ...
    protected void launchBoltAt(final lq targetedEntity) {
        final float bodyFacingAngle = this.ax * 3.141593f / 180.0f;
    ...
    protected void popNearbyMob() {
        final List nearbyMobs = this.p.b((lq)this, aoe.a().a(this.t, this.u, this.v, this.t + 1.0, this.u + 1.0, this.v + 1.0).b(32.0, 16.0, 32.0));
    ...
    protected void spawnShadowClone(final lq targetedEntity) {
        final aoj cloneSpot = this.findVecInLOSOf(targetedEntity);
        ...
        newClone.aZ = 60 + this.aa.nextInt(3) - this.aa.nextInt(3);
        this.makeTeleportTrail(this.t, this.u, this.v, cloneSpot.c, cloneSpot.d, cloneSpot.e);
    }
    
    protected void despawnClones() {
        final List nearbyLiches = this.p.a((Class)this.getClass(), aoe.a().a(this.t, this.u, this.v, this.t + 1.0, this.u + 1.0, this.v + 1.0).b(32.0, 16.0, 32.0));
        for (final EntityTFLich nearbyLich : nearbyLiches) {
    ...
    protected void checkAndSpawnMinions(final lq targetedEntity) {
        if (!this.p.I && this.getMinionsToSummon() > 0) {
    ...
    protected int countMyMinions() {
        final List nearbyMinons = this.p.a((Class)EntityTFLichMinion.class, aoe.a().a(this.t, this.u, this.v, this.t + 1.0, this.u + 1.0, this.v + 1.0).b(32.0, 16.0, 32.0));
        int count = 0;
    ...
    protected void spawnMinionAt(final md targetedEntity) {
        final aoj minionSpot = this.findVecInLOSOf((lq)targetedEntity);
    ...
    protected void teleportToSightOfEntity(final lq entity) {
        final aoj dest = this.findVecInLOSOf(entity);
    ...
    protected aoj findVecInLOSOf(final lq targetEntity) {
        double tx = 0.0;
        double ty = 0.0;
        double tz = 0.0;
        final int tries = 100;
        for (int i = 0; i < tries; ++i) {
            ...
            while (!groundFlag && ty > 0.0) {
                ...
                if (whatsThere == 0 || !amq.p[whatsThere].cB.a()) {
                    --ty;
                    --by;
                }
                else {
            ...
            if (by != 0) {
                if (this.canEntitySee(targetEntity, tx, ty, tz)) {
                    ...
                    final aoe destBox = aoe.a(tx - halfWidth, ty - this.M + this.W, tz - halfWidth, tx + halfWidth, ty - this.M + this.W + this.O, tz + halfWidth);
    ...
    static {
        heldItems = new ur[] { new ur(TFItems.scepterTwilight, 1), new ur(TFItems.scepterZombie, 1), new ur(up.G, 1) };
    }
}
