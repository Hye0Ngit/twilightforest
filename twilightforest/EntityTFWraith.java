...
public class EntityTFWraith extends ma implements qe
{
    ...
    public EntityTFWraith(final xv world) {
        super(world);
        this.aF = "/twilightforest/ghost-crown.png";
        this.bG = 0.5f;
        this.attackStrength = 5;
    }
    
    public EntityTFWraith(final xv world, final double x, final double y, final double z) {
        this(world);
        this.b(x, y, z);
    }
    ...
    protected void attackEntity(final lq entity, final float f) {
        if (this.aY <= 0 && f < 2.0f && entity.D.e > this.D.b && entity.D.b < this.D.e) {
            this.aY = 20;
    ...
    private boolean isCourseTraversable(final double d, final double d1, final double d2, final double d3) {
        ...
        final anw axisalignedbb = this.D.c();
        for (int i = 1; i < d3; ++i) {
