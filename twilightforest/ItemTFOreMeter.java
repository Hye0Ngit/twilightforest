...
public class ItemTFOreMeter extends ItemTF
{
    ...
    private void countOreInChunk(final qx player, final xv world, final int useX, final int useZ) {
        final int chunkX = useX >> 4;
        final int chunkZ = useZ >> 4;
        final int countStone = this.countBlockInChunk(world, amj.w.cm, chunkX, chunkZ);
        final int countDirt = this.countBlockInChunk(world, amj.y.cm, chunkX, chunkZ);
        final int countGravel = this.countBlockInChunk(world, amj.I.cm, chunkX, chunkZ);
        final int countCoal = this.countBlockInChunk(world, amj.L.cm, chunkX, chunkZ);
        final int countIron = this.countBlockInChunk(world, amj.K.cm, chunkX, chunkZ);
        final int countGold = this.countBlockInChunk(world, amj.J.cm, chunkX, chunkZ);
        final int countDiamond = this.countBlockInChunk(world, amj.az.cm, chunkX, chunkZ);
        final int countLapis = this.countBlockInChunk(world, amj.Q.cm, chunkX, chunkZ);
        final int countRedstone = this.countBlockInChunk(world, amj.aQ.cm, chunkX, chunkZ);
    ...
    private void countOreInArea(final qx player, final xv world, final int useX, final int useZ, final int radius) {
        final int chunkX = useX >> 4;
        final int chunkZ = useZ >> 4;
        int countStone = 0;
        int countDirt = 0;
        ...
        for (int cx = chunkX - radius; cx <= chunkX + radius; ++cx) {
            for (int cz = chunkZ - radius; cz <= chunkZ + radius; ++cz) {
                countStone += this.countBlockInChunk(world, amj.w.cm, chunkX, chunkZ);
                countDirt += this.countBlockInChunk(world, amj.y.cm, chunkX, chunkZ);
                countGravel += this.countBlockInChunk(world, amj.I.cm, chunkX, chunkZ);
                countCoal += this.countBlockInChunk(world, amj.L.cm, chunkX, chunkZ);
                countIron += this.countBlockInChunk(world, amj.K.cm, chunkX, chunkZ);
                countGold += this.countBlockInChunk(world, amj.J.cm, chunkX, chunkZ);
                countDiamond += this.countBlockInChunk(world, amj.az.cm, chunkX, chunkZ);
                countLapis += this.countBlockInChunk(world, amj.Q.cm, chunkX, chunkZ);
                countRedstone += this.countBlockInChunk(world, amj.aQ.cm, chunkX, chunkZ);
    ...
    public int countBlockInChunk(final xv world, final int blockID, final int cx, final int cz) {
        final zs chunk = world.e(cx, cz);
        int count = 0;
        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                for (int y = 0; y < 256; ++y) {
    ...
    public int countBlockInChunk(final xv world, final int blockID, final int meta, final int cx, final int cz) {
        final zs chunk = world.e(cx, cz);
        int count = 0;
        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
