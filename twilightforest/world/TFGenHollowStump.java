...
public class TFGenHollowStump extends TFGenHollowTree
{
    ...
    @Override
    protected void buildTrunk(final abv world, final Random random, final int x, final int y, final int z, final int diameter, final int maxHeight) {
            ...
            for (int dz = -diameter; dz <= diameter; ++dz) {
                for (int height = 2 + random.nextInt(3) + random.nextInt(2), dy2 = 0; dy2 <= height; ++dy2) {
                    ...
                    if (dist2 <= diameter && dist2 > hollow) {
                        this.putBlockAndMetadata(world, dx + x, dy2 + y, dz + z, this.treeBlock, this.treeMeta, true);
                    }
                }
            }
        }
