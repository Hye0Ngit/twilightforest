...
public class TFGenMonolith extends TFGenerator
{
    ...
    public boolean a(final abv world, final Random rand, final int x, final int y, final int z) {
        ...
        for (int cy = 0; cy <= h0; ++cy) {
            this.putBlock(world, x + 0, y + cy - 1, z + 0, (cy == ht) ? ((byte)aqw.T.cF) : ((byte)aqw.au.cF), true);
        }
        for (int cy = 0; cy <= h2; ++cy) {
            this.putBlock(world, x + 1, y + cy - 1, z + 0, (cy == ht) ? ((byte)aqw.T.cF) : ((byte)aqw.au.cF), true);
        }
        for (int cy = 0; cy <= h3; ++cy) {
            this.putBlock(world, x + 0, y + cy - 1, z + 1, (cy == ht) ? ((byte)aqw.T.cF) : ((byte)aqw.au.cF), true);
        }
        for (int cy = 0; cy <= h4; ++cy) {
            this.putBlock(world, x + 1, y + cy - 1, z + 1, (cy == ht) ? ((byte)aqw.T.cF) : ((byte)aqw.au.cF), true);
        }
        for (int i = 0; i < 2; ++i) {
