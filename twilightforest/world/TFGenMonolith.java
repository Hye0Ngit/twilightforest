...
public class TFGenMonolith extends TFGenerator
{
    public boolean func_76484_a(final World world, final Random rand, final int x, final int y, final int z) {
        ...
        if (!this.isAreaSuitable(world, rand, x, y, z, 2, ht, 2)) {
            return false;
        }
        int h0 = 0;
        int h2 = 0;
        ...
        for (int cy = 0; cy <= h0; ++cy) {
            this.setBlock(world, x + 0, y + cy - 1, z + 0, (cy == ht) ? Blocks.field_150368_y : Blocks.field_150343_Z);
        }
        for (int cy = 0; cy <= h2; ++cy) {
            this.setBlock(world, x + 1, y + cy - 1, z + 0, (cy == ht) ? Blocks.field_150368_y : Blocks.field_150343_Z);
        }
        for (int cy = 0; cy <= h3; ++cy) {
            this.setBlock(world, x + 0, y + cy - 1, z + 1, (cy == ht) ? Blocks.field_150368_y : Blocks.field_150343_Z);
        }
        for (int cy = 0; cy <= h4; ++cy) {
            this.setBlock(world, x + 1, y + cy - 1, z + 1, (cy == ht) ? Blocks.field_150368_y : Blocks.field_150343_Z);
        }
        for (int i = 0; i < 2; ++i) {
