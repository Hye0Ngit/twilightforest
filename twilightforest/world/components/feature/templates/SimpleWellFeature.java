...
public class SimpleWellFeature extends TemplateFeature<NoneFeatureConfiguration>
{
    ...
    public SimpleWellFeature(final Codec<NoneFeatureConfiguration> config) {
        super(config);
    }
    ...
    protected StructureTemplate getTemplate(final StructureManager templateManager, final Random random) {
        return templateManager.m_74341_(SimpleWellFeature.WELL_TOP);
    }
    ...
    protected void postPlacement(final WorldGenLevel world, final Random random, final StructureManager templateManager, final Rotation rotation, final Mirror mirror, final StructurePlaceSettings placementSettings, BlockPos placementPos) {
        final StructureTemplate template = templateManager.m_74341_(SimpleWellFeature.WELL_BOTTOM);
        if (template == null) {
            return;
        }
    ...
    protected void processMarkers(final StructureTemplate.StructureBlockInfo info, final WorldGenLevel world, final Rotation rotation, final Mirror mirror, final Random random) {
        ...
        Direction direction = switch (substring) {
    ...
    static {
        WELL_TOP = TwilightForestMod.prefix("feature/well/simple_well_top");
        WELL_BOTTOM = TwilightForestMod.prefix("feature/well/simple_well_bottom");
    }
}
