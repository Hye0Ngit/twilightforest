...
public class TFGenHollowTree extends TFGenerator
{
        ...
        for (int crownRadius = diameter * 4 + 8, dx = -crownRadius; dx <= crownRadius; ++dx) {
            for (int dz = -crownRadius; dz <= crownRadius; ++dz) {
                for (int dy = height - crownRadius; dy <= height + crownRadius; ++dy) {
                    final int whatsThere = world.func_72798_a(dx + x, dy + y, dz + z);
                    if (whatsThere != 0 && whatsThere != Block.field_71952_K.field_71990_ca) {
                        return false;
                    }
                }
            }
        ...
        for (int dx = -diameter; dx <= diameter; ++dx) {
            for (int dz = -diameter; dz <= diameter; ++dz) {
                for (int dy = -4; dy < 0; ++dy) {
                    ...
                    if (dist <= diameter) {
                        if (this.hasAirAround((IBlockAccess)world, dx + x, dy + y, dz + z)) {
                            this.func_76485_a(world, dx + x, dy + y, dz + z, this.treeBlock, (dist > hollow) ? this.treeMeta : this.branchMeta);
                        }
                        else {
                            this.func_76485_a(world, dx + x, dy + y, dz + z, this.rootBlock, this.rootMeta);
                        }
                    }
                }
            }
            ...
            for (int dz = -diameter; dz <= diameter; ++dz) {
                for (int dy = 0; dy <= height; ++dy) {
                    ...
                    if (dist <= diameter && dist > hollow) {
                        this.func_76485_a(world, dx + x, dy + y, dz + z, this.treeBlock, this.treeMeta);
                    }
                    if (dist <= hollow) {}
                    if (dist == hollow && dx == hollow) {
                        this.func_76485_a(world, dx + x, dy + y, dz + z, Block.field_71998_bu.field_71990_ca, 8);
                    }
                }
            }
        }
    ...
    protected void makeRoot(final World world, final Random random, final int x, final int y, final int z, final int diameter, final int branchHeight, final double length, final double angle, final double tilt) {
        ...
        for (final ChunkCoordinates coord : lineArray) {
            if (stillAboveGround && this.hasAirAround((IBlockAccess)world, coord.field_71574_a, coord.field_71572_b, coord.field_71573_c)) {
                this.func_76485_a(world, coord.field_71574_a, coord.field_71572_b, coord.field_71573_c, this.treeBlock, this.branchMeta);
                this.func_76485_a(world, coord.field_71574_a, coord.field_71572_b - 1, coord.field_71573_c, this.treeBlock, this.branchMeta);
            }
            else {
                this.func_76485_a(world, coord.field_71574_a, coord.field_71572_b, coord.field_71573_c, this.rootBlock, this.rootMeta);
                this.func_76485_a(world, coord.field_71574_a, coord.field_71572_b - 1, coord.field_71573_c, this.rootBlock, this.rootMeta);
    ...
    private void makeLeafDungeon(final World world, final Random random, final int x, final int y, final int z) {
        ...
        this.drawBlob(world, x, y, z, 2, 0, 0);
        world.func_72832_d(x + 0, y + 1, z + 0, Block.field_72065_as.field_71990_ca, 0, 2);
        final TileEntityMobSpawner ms = (TileEntityMobSpawner)world.func_72796_p(x + 0, y + 1, z + 0);
        if (ms != null) {
            ms.func_98049_a().func_98272_a(TFCreatures.getSpawnerNameFor("Swarm Spider"));
        }
    ...
    protected void addFirefly(final World world, final int x, final int y, final int z, final int diameter, final int fHeight, double fAngle) {
        ...
        if (TFBlocks.firefly.func_71930_b(world, src[0], src[1], src[2])) {
            this.func_76485_a(world, src[0], src[1], src[2], TFBlocks.firefly.field_71990_ca, tmeta);
        }
    }
    ...
    protected void addCicada(final World world, final int x, final int y, final int z, final int diameter, final int fHeight, double fAngle) {
        ...
        if (TFBlocks.cicada.func_71930_b(world, src[0], src[1], src[2])) {
            this.func_76485_a(world, src[0], src[1], src[2], TFBlocks.cicada.field_71990_ca, tmeta);
        }
    }
}
