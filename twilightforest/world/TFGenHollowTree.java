...
public class TFGenHollowTree extends TFGenerator
{
        ...
        for (int crownRadius = diameter * 4 + 8, dx = -crownRadius; dx <= crownRadius; ++dx) {
            for (int dz = -crownRadius; dz <= crownRadius; ++dz) {
                for (int dy = height - crownRadius; dy <= height + crownRadius; ++dy) {
                    ...
                    if (whatsThere != 0 && whatsThere != aqw.P.cF) {
                        return false;
                    }
                }
            }
    ...
    protected void buildFullCrown(final abv world, final Random random, final int x, final int y, final int z, final int diameter, final int height) {
    ...
    protected void buildWeakCrown(final abv world, final Random random, final int x, final int y, final int z, final int diameter, final int height) {
    ...
    protected void buildBranchRing(final abv world, final Random random, final int x, final int y, final int z, final int diameter, final int branchHeight, final int heightVar, final int length, final int lengthVar, final double tilt, final double tiltVar, final int minBranches, final int maxBranches, final int size, final boolean leafy) {
    ...
    protected void buildTrunk(final abv world, final Random random, final int x, final int y, final int z, final int diameter, final int height) {
        ...
        for (int dx = -diameter; dx <= diameter; ++dx) {
            for (int dz = -diameter; dz <= diameter; ++dz) {
                for (int dy = -4; dy < 0; ++dy) {
                    ...
                    if (dist <= diameter) {
                        if (this.hasAirAround((ace)world, dx + x, dy + y, dz + z)) {
                            this.a(world, dx + x, dy + y, dz + z, this.treeBlock, (dist > hollow) ? this.treeMeta : this.branchMeta);
                        }
                        else {
            ...
            for (int dz = -diameter; dz <= diameter; ++dz) {
                for (int dy = 0; dy <= height; ++dy) {
                    ...
                    if (dist == hollow && dx == hollow) {
                        this.putBlockAndMetadata(world, dx + x, dy + y, dz + z, aqw.bz.cF, 8, true);
                    }
                }
            }
        }
    ...
    protected void makeMedBranch(final abv world, final Random random, final int x, final int y, final int z, final int diameter, final int branchHeight, final double length, final double angle, final double tilt, final boolean leafy) {
        final int sx = x;
    ...
    protected void makeMedBranch(final abv world, final Random random, final int sx, final int sy, final int sz, final double length, final double angle, final double tilt, final boolean leafy) {
    ...
    protected void makeSmallBranch(final abv world, final Random random, final int sx, final int sy, final int sz, final double length, final double angle, final double tilt, final boolean leafy) {
        final int[] src = { sx, sy, sz };
        final int[] dest = TFGenerator.translate(src[0], src[1], src[2], length, angle, tilt);
        this.drawBresehnam(world, src[0], src[1], src[2], dest[0], dest[1], dest[2], this.treeBlock, this.branchMeta, true);
        if (leafy) {
    ...
    protected void makeSmallBranch(final abv world, final Random random, final int x, final int y, final int z, final int diameter, final int branchHeight, final double length, final double angle, final double tilt, final boolean leafy) {
        final int sx = x;
        final int sy = y + branchHeight;
        final int sz = z;
        final int[] src = TFGenerator.translate(sx, sy, sz, diameter, angle, 0.5);
    ...
    protected void makeRoot(final abv world, final Random random, final int x, final int y, final int z, final int diameter, final int branchHeight, final double length, final double angle, final double tilt) {
    ...
    protected void makeLargeBranch(final abv world, final Random random, final int sx, final int sy, final int sz, final double length, final double angle, final double tilt, final boolean leafy) {
        final int[] src = { sx, sy, sz };
        final int[] dest = TFGenerator.translate(src[0], src[1], src[2], length, angle, tilt);
        this.drawBresehnam(world, src[0], src[1], src[2], dest[0], dest[1], dest[2], this.treeBlock, this.branchMeta, true);
    ...
    private void makeLeafDungeon(final abv world, final Random random, final int x, final int y, final int z) {
        this.drawBlob(world, x, y, z, 4, this.leafBlock, this.leafMeta, true);
        this.drawBlob(world, x, y, z, 3, this.treeBlock, this.branchMeta, true);
        this.drawBlob(world, x, y, z, 2, 0, 0, true);
        world.f(x + 0, y + 1, z + 0, aqw.ax.cF, 0, 2);
        final asg ms = (asg)world.r(x + 0, y + 1, z + 0);
        if (ms != null) {
            ms.a().a(TFCreatures.getSpawnerNameFor("Swarm Spider"));
        }
        this.makeLeafDungeonChest(world, random, x, y, z);
    }
    
    private void makeLeafDungeonChest(final abv world, final Random random, int x, final int y, int z) {
    ...
    protected void makeLargeBranch(final abv world, final Random random, final int x, final int y, final int z, final int diameter, final int branchHeight, final double length, final double angle, final double tilt, final boolean leafy) {
        final int sx = x;
        final int sy = y + branchHeight;
        final int sz = z;
        final int[] src = TFGenerator.translate(sx, sy, sz, diameter, angle, 0.5);
    ...
    protected void addFirefly(final abv world, final int x, final int y, final int z, final int diameter, final int fHeight, double fAngle) {
    ...
    protected void addCicada(final abv world, final int x, final int y, final int z, final int diameter, final int fHeight, double fAngle) {
        final int[] src = TFGenerator.translate(x, y + fHeight, z, diameter + 1, fAngle, 0.5);
        fAngle %= 1.0;
        int tmeta = 1;
        if (fAngle > 0.875 || fAngle <= 0.125) {
