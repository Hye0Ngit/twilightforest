...
public class TFGenSmallTwilightOak extends TFGenerator
{
    ...
    @Override
    public boolean a(final abv par1World, final Random par2Random, final int x, final int y, final int z) {
            ...
            for (int cx = x - width; cx <= x + width && allClear; ++cx) {
                for (int cz = z - width; cz <= z + width && allClear; ++cz) {
                    if (cy >= 0 && cy < 256) {
                        ...
                        final aqw block = aqw.s[blockID];
                        if (blockID != 0 && !block.isLeaves(par1World, cx, cy, cz) && blockID != aqw.z.cF && blockID != aqw.A.cF && !block.isWood(par1World, cx, cy, cz)) {
                            allClear = false;
                        }
                    }
                    else {
        ...
        if ((cy == aqw.z.cF || cy == aqw.A.cF) && y < 256 - height - 1) {
            this.a(par1World, x, y - 1, z, aqw.A.cF);
            final byte width = 3;
            final byte var18 = 0;
            for (int cz = y - width + height; cz <= y + height; ++cz) {
                ...
                for (int treeWidth = var18 + 1 - blockID / 2, tx = x - treeWidth; tx <= x + treeWidth; ++tx) {
                    ...
                    for (int tz = z - treeWidth; tz <= z + treeWidth; ++tz) {
                        ...
                        final aqw block2 = aqw.s[par1World.a(tx, cz, tz)];
            ...
            for (int cz = 0; cz < height; ++cz) {
                ...
                final aqw block3 = aqw.s[blockID];
