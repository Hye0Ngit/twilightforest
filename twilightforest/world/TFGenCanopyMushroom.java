...
public class TFGenCanopyMushroom extends TFGenerator
{
    ...
    public boolean a(final xv world, final Random random, final int treeX, final int treeY, final int treeZ) {
        ...
        if ((blockUnder != amj.x.cm && blockUnder != amj.y.cm && blockUnder != amj.bB.cm) || this.y >= 128 - this.treeHeight - 1) {
            return false;
        }
        this.x = treeX;
        this.y = treeY;
        this.z = treeZ;
        this.treeBlock = ((this.treeRNG.nextInt(3) == 0) ? amj.br.cm : amj.bq.cm);
