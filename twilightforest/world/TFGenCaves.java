...
public class TFGenCaves extends MapGenCaves
{
    ...
    protected void generateLargeCaveNode(final long caveSeed, final int centerX, final int centerZ, final ChunkPrimer blockStorage, final double randX, final double randY, final double randZ, final boolean isHighlands) {
    ...
    protected void generateCaveNode(final long caveSeed, final int centerX, final int centerZ, final ChunkPrimer blockStorage, double randX, double randY, double randZ, float caveSize, float randPI, float angleToGenerate, int loopOne, int loopEnd, final double yScale, final boolean isHighlands) {
                ...
                if (randX >= offsetCenterX - 16.0 - sizeVar * 2.0 && randZ >= offsetCenterZ - 16.0 - sizeVar * 2.0 && randX <= offsetCenterX + 16.0 + sizeVar * 2.0 && randZ <= offsetCenterZ + 16.0 + sizeVar * 2.0) {
                    ...
                    if (!hasHitWater) {
                        for (int genX = minX; genX < maxX; ++genX) {
                            final double dx = (genX + centerX * 16 + 0.5 - randX) / sizeVar;
                            for (int genZ2 = minZ; genZ2 < maxZ; ++genZ2) {
                                final double dz = (genZ2 + centerZ * 16 + 0.5 - randZ) / sizeVar;
                                boolean hitGrass = false;
                                if (dx * dx + dz * dz < 1.0) {
                                    for (int caveY = minY - 1; caveY >= maxY; --caveY) {
                                        final double dy = (caveY + 0.5 - randY) / scaledSize;
                                        if (dy > -0.7 && dx * dx + dy * dy + dz * dz < 20.0) {
                                            final IBlockState blockStateAt = blockStorage.func_177856_a(genX, caveY, genZ2);
                                            final Block blockAt = blockStateAt.func_177230_c();
                                            if (blockAt == Blocks.field_150349_c) {
                                                hitGrass = true;
                                            }
                                            if (blockAt == Blocks.field_150348_b || blockAt == TFBlocks.trollsteinn || this.canReplace(blockStateAt.func_185904_a())) {
                                                if (dx * dx + dy * dy + dz * dz < 0.85) {
                                                    final IBlockState state = ((Block)((caveY < 10) ? Blocks.field_150355_j : Blocks.field_150350_a)).func_176223_P();
                                                    blockStorage.func_177855_a(genX, caveY, genZ2, state);
                                                }
                                                else {
                                                    Block localBlock = (mossRNG.nextInt(6) == 0) ? TFBlocks.trollsteinn : Blocks.field_150348_b;
                                                    localBlock = (isHighlands ? localBlock : Blocks.field_150346_d);
                                                    localBlock = (Block)(hitGrass ? Blocks.field_150349_c : localBlock);
                                                    blockStorage.func_177855_a(genX, caveY, genZ2, localBlock.func_176223_P());
                                                    hitGrass = false;
                                                }
                                                if (hitGrass && blockStorage.func_177856_a(genX, caveY - 1, genZ2).func_177230_c() == Blocks.field_150346_d) {
                                                    final IBlockState blockState = this.getBiome(genX, genZ2).field_76752_A;
                                                    blockStorage.func_177855_a(genX, caveY - 1, genZ2, blockState);
                                                }
                                            }
                                        }
                                    }
    ...
    private boolean canReplace(final Material material) {
        return material == Material.field_151578_c || material == Material.field_151577_b;
    }
    
    protected void func_180701_a(final World world, final int genX, final int genZ, final int centerX, final int centerZ, final ChunkPrimer primer) {
        final int numberOfCaves = this.field_75038_b.nextInt(this.field_75038_b.nextInt(this.field_75038_b.nextInt(40) + 1) + 1);
        if (this.field_75038_b.nextInt(15) != 0) {
            return;
        }
        this.biomes = world.func_72959_q().func_76933_b(this.biomes, genX << 4, genZ << 4, 16, 16);
        final boolean isHighlands = this.getBiome(0, 0) instanceof TFBiomeHighlands;
        for (int i = 0; i < numberOfCaves; ++i) {
            ...
            if (this.field_75038_b.nextInt(4) == 0) {
                this.generateLargeCaveNode(this.field_75038_b.nextLong(), centerX, centerZ, primer, randX, randY, randZ, isHighlands);
            ...
            for (int j = 0; j < numberOfNormalNodes; ++j) {
                ...
                this.generateCaveNode(this.field_75038_b.nextLong(), centerX, centerZ, primer, randX, randY, randZ, caveSize, randPi, randEight, 0, 0, 1.0, isHighlands);
            }
        }
    }
    
    private boolean isOceanBlock(final ChunkPrimer data, final int x, final int y, final int z) {
