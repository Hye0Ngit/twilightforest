...
public class TFGenHollowHill extends TFGenerator
{
    ...
    public boolean a(final xv world, final Random rand, final int x, final int y, final int z) {
        ...
        for (int i = 0; i < this.sn; ++i) {
            final int[] dest = this.getCoordsInHill2D();
            new TFGenCaveStalactite(amj.w.cm, rand.nextDouble(), true).a(this.worldObj, rand, dest[0], this.hy + 1, dest[1]);
        }
        for (int i = 0; i < this.sn; ++i) {
            final int[] dest = this.getCoordsInHill2D();
            new TFGenCaveStalactite(amj.w.cm, rand.nextDouble() * 0.7, false).a(this.worldObj, rand, dest[0], this.hy + 1, dest[1]);
        }
        if (this.hsize == 3) {
    ...
    protected boolean placeMobSpawner(final int dx, final int dy, final int dz) {
        this.worldObj.e(dx, dy, dz, amj.av.cm);
        final anl ms = (anl)this.worldObj.q(dx, dy, dz);
        if (ms != null) {
    ...
    protected boolean placeTreasureChestOld(final int dx, final int dy, final int dz) {
        this.worldObj.e(dx, dy, dz, amj.ax.cm);
        final anf tec = (anf)this.worldObj.q(dx, dy, dz);
    ...
    protected um getTreasure(final int level) {
        if (level == 1) {
            switch (this.hillRNG.nextInt(6)) {
                case 0: {
                    return new um(uk.o, this.hillRNG.nextInt(4) + 1);
                }
                case 1: {
                    return new um(uk.aw);
                }
                ...
                case 4: {
                    return new um(uk.T, this.hillRNG.nextInt(3) + 1);
                }
                default: {
                    return new um(amj.at, this.hillRNG.nextInt(16) + 1);
                }
            }
        }
        else if (level == 2) {
            switch (this.hillRNG.nextInt(8)) {
                ...
                case 4: {
                    return new um(uk.p, this.hillRNG.nextInt(6) + 1);
                }
                case 5: {
                    return new um(uk.aA);
                }
                case 6: {
                    return new um(uk.aW, this.hillRNG.nextInt(10) + 1, this.hillRNG.nextInt(16));
                }
                default: {
                    return new um(uk.F);
                }
            ...
            if (level != 3) {
                return new um(uk.m, this.hillRNG.nextInt(16) + 1);
            }
            switch (this.hillRNG.nextInt(8)) {
                case 0:
                case 1:
