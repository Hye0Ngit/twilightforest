...
public class TFGenRavine extends MapGenBase4096
{
    ...
    protected void generateRavine(final long l, final int i, final int j, final short[] shortStorage, double d, double d1, double d2, final float f, float f1, float f2, int k, int i1, final double d3) {
        ...
        while (k < i1) {
            double d6 = 1.5 + ke.a(k * 3.141593f / i1) * f * 1.0f;
            double d7 = d6 * d3;
            d6 *= random.nextFloat() * 0.25 + 0.75;
            d7 *= random.nextFloat() * 0.25 + 0.75;
            final float f6 = ke.b(f2);
            final float f7 = ke.a(f2);
            d += ke.b(f1) * f6;
            d1 += f7;
            d2 += ke.a(f1) * f6;
            f2 *= 0.7f;
            ...
            if (flag || random.nextInt(4) != 0) {
                ...
                if (d >= d4 - 16.0 - d6 * 2.0 && d2 >= d5 - 16.0 - d6 * 2.0 && d <= d4 + 16.0 + d6 * 2.0) {
                    if (d2 <= d5 + 16.0 + d6 * 2.0) {
                        int d9 = ke.c(d - d6) - i * 16 - 1;
                        int l2 = ke.c(d + d6) - i * 16 + 1;
                        int d10 = ke.c(d1 - d7) - 1;
                        int i2 = ke.c(d1 + d7) + 1;
                        int d11 = ke.c(d2 - d6) - j * 16 - 1;
                        int j3 = ke.c(d2 + d6) - j * 16 + 1;
                        if (d9 < 0) {
                            d9 = 0;
                        }
                        if (l2 > 16) {
                                ...
                                for (int j4 = i2 + 1; !flag2 && j4 >= d10 - 1; --j4) {
                                    ...
                                    if (j4 >= 0) {
                                        if (j4 < TFWorld.WORLDHEIGHT) {
                                            if (shortStorage[k4] == amj.D.cm || shortStorage[k4] == amj.E.cm) {
                                                flag2 = true;
                                            }
                                    ...
                                    if (d12 * d12 + d13 * d13 < 1.0) {
                                        for (int j5 = i2 - 1; j5 >= d10; --j5) {
                                            ...
                                            if ((d12 * d12 + d13 * d13) * this.field_35627_a[j5] + d14 * d14 / 6.0 < 1.0) {
                                                ...
                                                if (curentBlock == amj.x.cm) {
                                                    flag3 = true;
                                                }
                                                if (curentBlock == amj.w.cm || curentBlock == amj.y.cm || curentBlock == amj.x.cm) {
                                                    shortStorage[i4] = 0;
                                                    if (flag3 && shortStorage[i4 - 1] == amj.y.cm) {
    ...
    @Override
    protected void recursiveGenerate(final xv world, final int centerChunkX, final int centerChunkZ, final int currentChunkX, final int currentChunkZ, final short[] shortStorage) {
