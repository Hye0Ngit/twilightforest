...
public class TFGenRavine extends MapGenBase4096
{
                                ...
                                for (int j4 = i2 + 1; !flag2 && j4 >= d10 - 1; --j4) {
                                    ...
                                    if (j4 >= 0) {
                                        if (j4 < TFWorld.WORLDHEIGHT) {
                                            if (shortStorage[k4] == amq.D.cm || shortStorage[k4] == amq.E.cm) {
                                                flag2 = true;
                                            }
                                    ...
                                    if (d12 * d12 + d13 * d13 < 1.0) {
                                        for (int j5 = i2 - 1; j5 >= d10; --j5) {
                                            ...
                                            if ((d12 * d12 + d13 * d13) * this.field_35627_a[j5] + d14 * d14 / 6.0 < 1.0) {
                                                ...
                                                if (curentBlock == amq.x.cm) {
                                                    flag3 = true;
                                                }
                                                if (curentBlock == amq.w.cm || curentBlock == amq.y.cm || curentBlock == amq.x.cm) {
                                                    shortStorage[i4] = 0;
                                                    if (flag3 && shortStorage[i4 - 1] == amq.y.cm) {
    ...
    @Override
    protected void recursiveGenerate(final yc world, final int centerChunkX, final int centerChunkZ, final int currentChunkX, final int currentChunkZ, final short[] shortStorage) {
