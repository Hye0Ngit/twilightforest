...
public class ChunkProviderTwilightForest implements IChunkProvider
{
    ...
    public Chunk func_73154_d(final int cx, final int cz) {
        ...
        this.hollowTreeGenerator.func_75036_a((IChunkProvider)this, this.worldObj, cx, cz, fake);
    ...
    public void deformTerrainForFeature(final int cx, final int cz, final short[] storage) {
        ...
        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                ...
                if (nearFeature == TFFeature.hill1 || nearFeature == TFFeature.hill2 || nearFeature == TFFeature.hill3 || nearFeature == TFFeature.hydraLair) {
                    ...
                    this.raiseHills(storage, nearFeature, hdiam, x, z, dx, dz, hheight);
                }
                if (nearFeature == TFFeature.hedgeMaze || nearFeature == TFFeature.nagaLair || nearFeature == TFFeature.questGrove) {
                    this.flattenTerrainForFeature(storage, nearFeature, x, z, dx, dz);
                }
                if (nearFeature == TFFeature.yetiCave) {
                    this.deformTerrainForYetiLair(storage, nearFeature, x, z, dx, dz);
                }
            }
        }
    }
    
    private void raiseHills(final short[] storage, final TFFeature nearFeature, final int hdiam, final int x, final int z, final int dx, final int dz, final int hheight) {
        int oldGround = -1;
        int newGround = -1;
        for (int y = 0; y <= 127; ++y) {
    ...
    private void flattenTerrainForFeature(final short[] storage, final TFFeature nearFeature, final int x, final int z, final int dx, final int dz) {
        ...
        if (squishfactor > 0.0f) {
            ...
            for (int y = 0; y <= 127; ++y) {
                final int index = (x * 16 + z) * TFWorld.WORLDHEIGHT + y;
                final int currentTerrain = storage[index];
                if (currentTerrain != Block.field_71981_t.field_71990_ca) {
                    if (newGround == -1) {
        ...
        for (int y = 0; y <= 127; ++y) {
            ...
            if (y >= mazeheight && storage[index] != Block.field_71943_B.field_71990_ca) {
                storage[index] = 0;
            }
        }
    }
    
    private void deformTerrainForYetiLair(final short[] storage, final TFFeature nearFeature, final int x, final int z, final int dx, final int dz) {
        float squishfactor = 0.0f;
        ...
        for (int y = 0; y <= 127; ++y) {
            final int index = (x * 16 + z) * TFWorld.WORLDHEIGHT + y;
            if (y < topHeight && (storage[index] == 0 || storage[index] == Block.field_71943_B.field_71990_ca)) {
                storage[index] = (short)Block.field_71981_t.field_71990_ca;
            }
            ...
            if (y == hollowFloor && y < hollowCeiling && y < TFWorld.SEALEVEL + 3) {
                storage[index] = (short)Block.field_72036_aT.field_71990_ca;
            }
        }
    }
        ...
        for (int z = 0; z < 16; ++z) {
            for (int x = 0; x < 16; ++x) {
                ...
                if (generateForest) {
                    int topLevel = -1;
                    for (int y = 127; y >= 0; --y) {
                        final int index = ((z & 0xF) | (x & 0xF) << 4) << 7 | (y & 0x7F);
                    ...
                    if (topLevel != -1) {
                        ...
                        for (int treeTop = treeBottom + 11, y2 = treeBottom; y2 <= treeTop; ++y2) {
                            final int index2 = ((z & 0xF) | (x & 0xF) << 4) << 7 | (y2 & 0x7F);
    ...
    public List<SpawnListEntry> func_73155_a(final EnumCreatureType creatureType, final int mapX, final int mapY, final int mapZ) {
    ...
    public void func_82695_e(final int var1, final int var2) {
        ...
        this.hollowTreeGenerator.func_75036_a((IChunkProvider)this, this.worldObj, var1, var2, (byte[])null);
    }
