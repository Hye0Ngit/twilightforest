...
public class ChunkProviderTwilightForest implements adn
{
    private Random rand;
    ...
    public ChunkProviderTwilightForest(final abv world, final long l, final boolean flag) {
                    ...
                    for (int j2 = 0; j2 < 8; ++j2) {
                        ...
                        for (int k2 = 0; k2 < 4; ++k2) {
                            ...
                            for (int i3 = 0; i3 < 4; ++i3) {
                                if ((d16 += d17) > 0.0) {
                                    par3ArrayofShort[l2 += c] = (short)aqw.y.cF;
                                }
                                else if (i2 * 8 + j2 < byte3) {
                                    par3ArrayofShort[l2 += c] = (short)aqw.G.cF;
                                }
                                else {
    ...
    public void replaceBlocksForBiome(final int par1, final int par2, final short[] blockArray4096, final acp[] par4ArrayOfBiomeGenBase) {
            ...
            for (int j = 0; j < 16; ++j) {
                ...
                for (int i2 = 127; i2 >= 0; --i2) {
                    ...
                    if (i2 <= 0 + this.rand.nextInt(5)) {
                        blockArray4096[j2] = (short)aqw.E.cF;
                    }
                    else {
                        final short currentBlock = blockArray4096[j2];
                        if (currentBlock == aqw.y.cF) {
                            if (l == -1) {
                                if (k <= 0) {
                                    top = 0;
    ...
    public adq c(final int i, final int j) {
        return this.d(i, j);
    }
    
    public adq d(final int cx, final int cz) {
        ...
        this.addGlaciers(cx, cz, blockStorage, metaStorage, this.biomesForGeneration = this.worldObj.u().b(this.biomesForGeneration, cx * 16, cz * 16, 16, 16));
        ...
        this.caveGenerator.generate((adn)this, this.worldObj, cx, cz, blockStorage);
        this.ravineGenerator.generate((adn)this, this.worldObj, cx, cz, blockStorage);
        final byte[] fake = new byte[0];
        this.majorFeatureGenerator.a((adn)this, this.worldObj, cx, cz, fake);
        final adq chunk = this.makeAChunk(this.worldObj, blockStorage, metaStorage, cx, cz);
    ...
    protected adq makeAChunk(final abv world, final short[] ids, final byte[] metadata, final int chunkX, final int chunkZ) {
        final adq chunk = new adq(world, chunkX, chunkZ);
        final adr[] storageArrays = chunk.i();
            ...
            for (int z = 0; z < 16; ++z) {
                ...
                if (nearFeature == TFFeature.hill1 || nearFeature == TFFeature.hill2 || nearFeature == TFFeature.hill3 || nearFeature == TFFeature.hydraLair) {
                    for (int y = 0; y <= 127; ++y) {
                        ...
                        if (currentTerrain == 0 || currentTerrain == aqw.aY.cF) {
                            if (newGround == -1) {
                                oldGround = y;
                    ...
                    for (int y2 = 0; y2 <= 127; ++y2) {
                        ...
                        if (hheight > 0 && y2 < TFWorld.SEALEVEL && storage[index2] != aqw.y.cF) {
                ...
                if (nearFeature == TFFeature.hedgeMaze || nearFeature == TFFeature.nagaLair || nearFeature == TFFeature.questGrove) {
                    ...
                    for (int y3 = 0; y3 <= 127; ++y3) {
                        final int index3 = (x * 16 + z) * TFWorld.WORLDHEIGHT + y3;
                        if (y3 < mazeheight && (storage[index3] == 0 || storage[index3] == aqw.G.cF)) {
                            storage[index3] = (short)aqw.y.cF;
                        }
                        if (y3 >= mazeheight && storage[index3] != aqw.G.cF) {
                            storage[index3] = 0;
                        }
                    }
                }
    ...
    private double[] makeLandPerBiome(double[] ad, int xx, final int zero, int zz, final int l, final int i1, final int j1) {
        ...
        if (this.squareTable == null) {
            ...
            for (int k1 = -2; k1 <= 2; ++k1) {
                for (int l2 = -2; l2 <= 2; ++l2) {
                    final float f = 10.0f / lr.c(k1 * k1 + l2 * l2 + 0.2f);
        ...
        for (int k2 = 0; k2 < l; ++k2) {
            for (int l3 = 0; l3 < j1; ++l3) {
                ...
                float f4 = 0.0f;
                final byte byte0 = 2;
                final acp biomegenbase = this.biomesForGeneration[k2 + 2 + (l3 + 2) * (l + 5)];
                for (int i3 = -byte0; i3 <= byte0; ++i3) {
                    for (int j3 = -byte0; j3 <= byte0; ++j3) {
                        final acp biomegenbase2 = this.biomesForGeneration[k2 + i3 + 2 + (l3 + j3 + 2) * (l + 5)];
    ...
    public void addGlaciers(final int chunkX, final int chunkZ, final short[] blocks, final byte[] meta, final acp[] biomes) {
        for (int x = 0; x < 16; ++x) {
            for (int z = 0; z < 16; ++z) {
                final acp biome = biomes[z + x * 16];
    ...
    public void addDarkForestCanopy(final int chunkX, final int chunkZ, final short[] blocks, final byte[] meta, final acp[] biomes) {
        final double d = 0.03125;
    ...
    public void a(final adn ichunkprovider, final int chunkX, final int chunkZ) {
        aop.c = true;
        int mapX = chunkX * 16;
        int mapY = chunkZ * 16;
        ...
        this.rand.setSeed(chunkX * l1 + chunkZ * l2 ^ this.worldObj.H());
        ...
        for (int k1 = 0; k1 < 8; ++k1) {
            ...
            if (!new afo().a(this.worldObj, this.rand, k2, k3, l3)) {}
        }
        biomegenbase.a(this.worldObj, this.rand, mapX, mapY);
        ach.a(this.worldObj, biomegenbase, mapX + 8, mapY + 8, 16, 16, this.rand);
        mapX += 8;
        mapY += 8;
        for (int i4 = 0; i4 < 16; ++i4) {
            for (int j4 = 0; j4 < 16; ++j4) {
                ...
                if (this.worldObj.x(i4 + mapX, j5 - 1, j4 + mapY)) {
                    this.worldObj.f(i4 + mapX, j5 - 1, j4 + mapY, aqw.aY.cF, 0, 2);
                }
                if (this.worldObj.z(i4 + mapX, j5, j4 + mapY)) {
                    this.worldObj.f(i4 + mapX, j5, j4 + mapY, aqw.aX.cF, 0, 2);
                }
            }
        }
        aop.c = false;
    ...
    public boolean a(final boolean flag, final lw iprogressupdate) {
        return true;
    }
    ...
    public List<acq> a(final og creatureType, final int mapX, final int mapY, final int mapZ) {
        ...
        final acp biomegenbase = this.worldObj.a(mapX, mapZ);
        if (biomegenbase == null) {
            return null;
        }
    ...
    public acn a(final abv par1World, final String par2Str, final int par3, final int par4, final int par5) {
        return null;
    }
    ...
    public void e(final int var1, final int var2) {
        this.majorFeatureGenerator.a((adn)this, this.worldObj, var1, var2, (byte[])null);
    }
