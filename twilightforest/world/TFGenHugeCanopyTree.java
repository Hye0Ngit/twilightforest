...
public class TFGenHugeCanopyTree extends TFGenerator
{
    ...
    public boolean a(final xv world, final Random random, final int treeX, final int treeY, final int treeZ) {
        ...
        if ((blockUnder != amj.x.cm && blockUnder != amj.y.cm) || this.y >= TFWorld.WORLDHEIGHT - this.treeHeight) {
            return false;
        }
        this.x = treeX;
        this.y = treeY;
    ...
    void buildBranch(final int height, final double length, final double angle, final double tilt, final boolean firefly) {
        ...
        this.drawBresehnam(src[0], src[1] - 1, src[2], dest[0], dest[1] - 1, dest[2], this.treeBlock, this.branchMeta, true);
        if (firefly) {
    ...
    protected void makeLeafNode(final int[] dest) {
        this.drawBresehnam(dest[0], dest[1], dest[2], dest[0] + 4, dest[1], dest[2] + 0, this.treeBlock, this.branchMeta, true);
        this.drawBresehnam(dest[0], dest[1], dest[2], dest[0] - 4, dest[1], dest[2] + 0, this.treeBlock, this.branchMeta, true);
        this.drawBresehnam(dest[0], dest[1], dest[2], dest[0] + 0, dest[1], dest[2] + 4, this.treeBlock, this.branchMeta, true);
        this.drawBresehnam(dest[0], dest[1], dest[2], dest[0] + 0, dest[1], dest[2] - 4, this.treeBlock, this.branchMeta, true);
        this.putBlockAndMetadata(dest[0] + 5, dest[1], dest[2] + 1, this.treeBlock, this.branchMeta, true);
        this.putBlockAndMetadata(dest[0] + 5, dest[1], dest[2] - 1, this.treeBlock, this.branchMeta, true);
        this.putBlockAndMetadata(dest[0] - 5, dest[1], dest[2] + 1, this.treeBlock, this.branchMeta, true);
        this.putBlockAndMetadata(dest[0] - 5, dest[1], dest[2] - 1, this.treeBlock, this.branchMeta, true);
        this.putBlockAndMetadata(dest[0] + 1, dest[1], dest[2] + 5, this.treeBlock, this.branchMeta, true);
        this.putBlockAndMetadata(dest[0] - 1, dest[1], dest[2] + 5, this.treeBlock, this.branchMeta, true);
        this.putBlockAndMetadata(dest[0] + 1, dest[1], dest[2] - 5, this.treeBlock, this.branchMeta, true);
        this.putBlockAndMetadata(dest[0] - 1, dest[1], dest[2] - 5, this.treeBlock, this.branchMeta, true);
        this.drawBresehnam(dest[0] + 1, dest[1], dest[2] + 0, dest[0] + 4, dest[1], dest[2] + 3, this.treeBlock, this.branchMeta, true);
        this.drawBresehnam(dest[0] - 1, dest[1], dest[2] + 0, dest[0] - 4, dest[1], dest[2] - 3, this.treeBlock, this.branchMeta, true);
        this.drawBresehnam(dest[0] + 0, dest[1], dest[2] + 1, dest[0] - 3, dest[1], dest[2] + 4, this.treeBlock, this.branchMeta, true);
        this.drawBresehnam(dest[0] + 0, dest[1], dest[2] - 1, dest[0] + 3, dest[1], dest[2] - 4, this.treeBlock, this.branchMeta, true);
    ...
    private void drawRootBlock(final int dx, final int dy, final int dz) {
        if (this.hasAirAround((yf)this.worldObj, dx, dy, dz)) {
            this.worldObj.c(dx, dy, dz, this.treeBlock, this.branchMeta);
        }
        else {
            this.worldObj.c(dx, dy, dz, this.rootBlock, this.rootMeta);
        }
    }
    ...
    protected void drawRoot(final int sx, final int sy, final int sz, final int dx, final int dy, final int dz) {
        if (this.worldObj.g(dx, dy, dz).a()) {
            ...
            for (int i = 0; i < lineArray.length; i += 3) {
                if (this.worldObj.a(lineArray[i + 0], lineArray[i + 1], lineArray[i + 2]) > 0 || this.isNearSolid((yf)this.worldObj, lineArray[i + 0], lineArray[i + 1], lineArray[i + 2])) {
