...
public class TFGenHugeCanopyTree extends TFGenerator
{
    ...
    public boolean a(final abv world, final Random random, final int x, final int y, final int z) {
        ...
        if ((blockUnder != aqw.z.cF && blockUnder != aqw.A.cF) || y >= TFWorld.MAXHEIGHT - treeHeight) {
            return false;
        }
    ...
    void buildBranch(final abv world, final int x, final int y, final int z, final int height, final double length, final double angle, final double tilt) {
    ...
    protected void makeLeafNode(final abv world, final int[] dest) {
    ...
    void buildTrunk(final abv world, final int x, final int y, final int z, final int height, final double length, final double angle, final double tilt, final boolean firefly) {
        final int[] src = { x, y + height, z };
        final int[] dest = TFGenerator.translate(src[0], src[1], src[2], length, angle, tilt);
    ...
    private void drawRootBlock(final abv world, final int dx, final int dy, final int dz) {
        if (this.hasAirAround((ace)world, dx, dy, dz)) {
    ...
    private void buildRoot(final abv world, final int x, final int y, final int z, final double offset, final int b) {
