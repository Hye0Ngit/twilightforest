...
public class TFGenWitchHut extends TFGenerator
{
    ...
    public boolean generateTinyHut(final World world, final Random rand, final int x, final int y, final int z) {
        if (!this.isAreaSuitable(world, rand, x, y, z, 5, 7, 6)) {
            return false;
        }
        ...
        final MobSpawnerBaseLogic spawnerLogic = ms.func_145881_a();
        if (spawnerLogic != null) {
            spawnerLogic.func_98272_a(TFCreatures.getSpawnerNameFor("Skeleton Druid"));
        }
        return true;
    }
}
