...
public abstract class TFGenerator extends abf
{
    protected xv worldObj;
    ...
    public abstract boolean a(final xv p0, final Random p1, final int p2, final int p3, final int p4);
    ...
    protected boolean putBlockAndMetadata(final int dx, final int dy, final int dz, final int blockValue, final int metaValue, final boolean priority) {
        if (priority) {
            this.worldObj.c(dx, dy, dz, blockValue, metaValue);
        }
        else {
    ...
    protected byte randStone(final Random rand, final int howMuch) {
        return (rand.nextInt(howMuch) >= 1) ? ((byte)amj.z.cm) : ((byte)amj.ar.cm);
    }
    
    protected boolean isAreaClear(final xv world, final Random rand, final int x, final int y, final int z, final int dx, final int dy, final int dz) {
        boolean flag = true;
        for (int cx = 0; cx < dx; ++cx) {
            for (int cz = 0; cz < dy; ++cz) {
                final agb m = world.g(x + cx, y - 1, z + cz);
                if (m != agb.c && m != agb.b && m != agb.e) {
                    flag = false;
                }
        ...
        for (int cx = 0; cx < width; ++cx) {
            for (int cy = 0; cy < height; ++cy) {
                for (int cz = 0; cz < depth; ++cz) {
                    this.worldObj.c(dx + cx, dy + cy, dz + cz, blockID, meta);
                }
            }
        }
    }
    ...
    protected boolean hasAirAround(final yf world, final int bx, final int by, final int bz) {
    ...
    protected boolean isNearSolid(final yf world, final int bx, final int by, final int bz) {
