...
public class TFGenDarkCanopyTree extends TFGenerator
{
    ...
    @Override
    public boolean a(final xv world, final Random random, final int treeX, int treeY, final int treeZ) {
        this.worldObj = world;
        this.treeRNG = random;
        boolean foundDirt = false;
        for (int dy = treeY; dy >= TFWorld.SEALEVEL; --dy) {
            final agb materialUnder = world.g(treeX, dy - 1, treeZ);
            if (materialUnder == agb.b || materialUnder == agb.c) {
                foundDirt = true;
                treeY = dy;
                break;
            }
            if (materialUnder == agb.e) {
                break;
            }
            if (materialUnder == agb.p) {
                break;
            }
        }
        if (!foundDirt) {
        ...
        if (world.g(treeX + 1, this.y, treeZ + 0) == agb.d || world.g(treeX - 1, this.y, treeZ + 0) == agb.d || world.g(treeX + 0, this.y, treeZ + 1) == agb.d || world.g(treeX + 0, this.y, treeZ - 1) == agb.d) {
            return false;
        }
        this.x = treeX;
        this.y = treeY;
        ...
        if (this.hasAirAround((yf)world, this.x, this.y - 1, this.z)) {
    ...
    void nullifySkyLightColumn(final int dx, final int dy, final int dz) {
        for (int y = dy; y >= 0; --y) {
            this.worldObj.b(yh.a, dx, y, dz, 0);
        }
    }
